...
#           icon_symlink_path.symlink_to(icon_path)
    ...
    def show_options_for_script(self, script, row):
        ...
        options = [
            ...
            ("Delete Shortcut", "edit-delete-symbolic", lambda sc, r=row: self.show_delete_shortcut_confirmation(sc, r), "Show confirmation for deleting the shortcut for the script"),
        ...
        for label, icon_name, callback, tooltip in options:
            ...
            option_button.connect("clicked", lambda btn, cb=callback, sc=script: cb(sc))
...
# NEW NEW NEW
    ...
    def show_delete_confirmation(self, script, option_button, row):
        self.replace_button_with_overlay(option_button, "Delete Wineprefix?", script, "wineprefix")

    def show_delete_shortcut_confirmation(self, script, option_button, row):
        self.replace_button_with_overlay(option_button, "Delete shortcut?", script, "shortcut")

    def show_wine_arguments(self, script, option_button, row):
        self.replace_button_with_entry_overlay(option_button, "Wine Arguments", script, row)


    def on_confirm_action(self, button, script, action_type, overlay, confirmation_box):
        print(f"Confirmed action: {action_type} for script {script}")
        overlay.remove_overlay(confirmation_box)
        # Implement the logic to handle the deletion or any action

    def on_cancel_button_clicked(self, button, overlay, confirmation_box, main_button, original_label):
        if confirmation_box.get_parent() == overlay:
            overlay.remove_overlay(confirmation_box)
        main_button.set_label(original_label)  # Restore the original label
        main_button.set_sensitive(True)  # Re-enable the button
        self.active_overlay = None  # Reset the active overlay

    def on_ok_button_clicked(self, button, overlay, confirmation_box, main_button, original_label, entry, script):
        print(f"Entry text: {entry.get_text()}")  # Handle the entry text here
        overlay.remove_overlay(confirmation_box)
        main_button.set_label(original_label)  # Restore the original label
        main_button.set_sensitive(True)  # Re-enable the button
        self.active_overlay = None  # Reset the active overlay
    ...
    def show_options_for_script(self, script, row):
        ...
        # Create "Open Terminal" button
        terminal_button = Gtk.Button()
        terminal_button.set_size_request(390, 36)
        terminal_button.add_css_class("flat")
        terminal_button.add_css_class("normal-font")
        terminal_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        terminal_button.set_child(terminal_hbox)
        terminal_icon = Gtk.Image.new_from_icon_name("utilities-terminal-symbolic")
        terminal_label = Gtk.Label(label="Open Terminal")
        terminal_label.set_xalign(0)
        terminal_label.set_hexpand(True)
        terminal_label.set_ellipsize(Pango.EllipsizeMode.END)
        terminal_hbox.append(terminal_icon)
        terminal_hbox.append(terminal_label)
        terminal_button.connect("clicked", lambda btn: self.open_terminal(script, terminal_button, row))

        # Create "Install dxvk vkd3d" button
        install_button = Gtk.Button()
        install_button.set_size_request(390, 36)
        install_button.add_css_class("flat")
        install_button.add_css_class("normal-font")
        install_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        install_button.set_child(install_hbox)
        install_icon = Gtk.Image.new_from_icon_name("emblem-system-symbolic")
        install_label = Gtk.Label(label="Install dxvk vkd3d")
        install_label.set_xalign(0)
        install_label.set_hexpand(True)
        install_label.set_ellipsize(Pango.EllipsizeMode.END)
        install_hbox.append(install_icon)
        install_hbox.append(install_label)
        install_button.connect("clicked", lambda btn: self.install_dxvk_vkd3d(script, install_button, row))

        # Create "Open Filemanager" button
        filemanager_button = Gtk.Button()
        filemanager_button.set_size_request(390, 36)
        filemanager_button.add_css_class("flat")
        filemanager_button.add_css_class("normal-font")
        filemanager_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        filemanager_button.set_child(filemanager_hbox)
        filemanager_icon = Gtk.Image.new_from_icon_name("system-file-manager-symbolic")
        filemanager_label = Gtk.Label(label="Open Filemanager")
        filemanager_label.set_xalign(0)
        filemanager_label.set_hexpand(True)
        filemanager_label.set_ellipsize(Pango.EllipsizeMode.END)
        filemanager_hbox.append(filemanager_icon)
        filemanager_hbox.append(filemanager_label)
        filemanager_button.connect("clicked", lambda btn: self.open_filemanager(script, filemanager_button, row))

        # Create "Delete Wineprefix" button
        delete_wineprefix_button = Gtk.Button()
        delete_wineprefix_button.set_size_request(390, 36)
        delete_wineprefix_button.add_css_class("flat")
        delete_wineprefix_button.add_css_class("normal-font")
        delete_wineprefix_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        delete_wineprefix_button.set_child(delete_wineprefix_hbox)
        delete_wineprefix_icon = Gtk.Image.new_from_icon_name("edit-delete-symbolic")
        delete_wineprefix_label = Gtk.Label(label="Delete Wineprefix")
        delete_wineprefix_label.set_xalign(0)
        delete_wineprefix_label.set_hexpand(True)
        delete_wineprefix_label.set_ellipsize(Pango.EllipsizeMode.END)
        delete_wineprefix_hbox.append(delete_wineprefix_icon)
        delete_wineprefix_hbox.append(delete_wineprefix_label)
        delete_wineprefix_button.connect("clicked", lambda btn: self.show_delete_confirmation(script, delete_wineprefix_button, row))

        # Create "Delete Shortcut" button
        delete_shortcut_button = Gtk.Button()
        delete_shortcut_button.set_size_request(390, 36)
        delete_shortcut_button.add_css_class("flat")
        delete_shortcut_button.add_css_class("normal-font")
        delete_shortcut_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        delete_shortcut_button.set_child(delete_shortcut_hbox)
        delete_shortcut_icon = Gtk.Image.new_from_icon_name("edit-delete-symbolic")
        delete_shortcut_label = Gtk.Label(label="Delete Shortcut")
        delete_shortcut_label.set_xalign(0)
        delete_shortcut_label.set_hexpand(True)
        delete_shortcut_label.set_ellipsize(Pango.EllipsizeMode.END)
        delete_shortcut_hbox.append(delete_shortcut_icon)
        delete_shortcut_hbox.append(delete_shortcut_label)
        delete_shortcut_button.connect("clicked", lambda btn: self.show_delete_shortcut_confirmation(script, delete_shortcut_button, row))

        # Create "Wine Arguments" button
        wine_arguments_button = Gtk.Button()
        wine_arguments_button.set_size_request(390, 36)
        wine_arguments_button.add_css_class("flat")
        wine_arguments_button.add_css_class("normal-font")
        wine_arguments_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        wine_arguments_button.set_child(wine_arguments_hbox)
        wine_arguments_icon = Gtk.Image.new_from_icon_name("preferences-system-symbolic")
        wine_arguments_label = Gtk.Label(label="Wine Arguments")
        wine_arguments_label.set_xalign(0)
        wine_arguments_label.set_hexpand(True)
        wine_arguments_label.set_ellipsize(Pango.EllipsizeMode.END)
        wine_arguments_hbox.append(wine_arguments_icon)
        wine_arguments_hbox.append(wine_arguments_label)
        wine_arguments_button.connect("clicked", lambda btn: self.show_wine_arguments(script, wine_arguments_button, row))

        # Append buttons to the options FlowBox
        options_flowbox.append(terminal_button)
        options_flowbox.append(install_button)
        options_flowbox.append(filemanager_button)
        options_flowbox.append(delete_wineprefix_button)
        options_flowbox.append(delete_shortcut_button)
        options_flowbox.append(wine_arguments_button)
    ...
    def replace_button_with_overlay(self, button, confirmation_text, script, action_type):
        # Get the parent, which should be a FlowBoxChild
        ...
        if isinstance(parent, Gtk.FlowBoxChild):
            ...
            parent.set_child(overlay)  # Set the overlay as the child of the FlowBoxChild

            # Connect the Yes and No buttons
    ...
    def replace_button_with_entry_overlay(self, button, prompt_text, script, row):
        # Get the parent, which should be a FlowBoxChild
        parent = button.get_parent()
        ...
        if isinstance(parent, Gtk.FlowBoxChild):
            ...
            entry = Gtk.Entry()
            entry.set_text("-opengl -SkipBuildPatchPrereq")
            entry.set_hexpand(True)
            entry_box.append(entry)

            button_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            # Create the OK button with a symbolic icon
            ok_button = Gtk.Button()
            ok_icon = Gtk.Image.new_from_icon_name("emblem-ok-symbolic")
            ok_button.set_child(ok_icon)

            # Create the Cancel button with a symbolic icon
            ...
            ok_button.connect("clicked", self.on_ok_button_clicked, parent, button, entry, script)
    ...
    def on_confirm_action(self, button, script, action_type, parent, original_button):
        print(f"Confirmed action: {action_type} for script {script}")
        # Restore the original button or take any other actions you want on confirmation
        parent.set_child(original_button)
        original_button.set_sensitive(True)
