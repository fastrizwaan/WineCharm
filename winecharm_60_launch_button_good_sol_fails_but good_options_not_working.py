...
#NEW2
    def launch_script(self, script, play_stop_button, row):
        ...
        try:
            ...
            # Store process information without play and options buttons
            self.running_processes[script.stem] = {
                "proc": proc,
                "pid": proc.pid,
                "pgid": pgid,
...
#            Here we should use pgrep to get pid for proc so that it can wait.
            def monitor_process():
                proc.wait()
                GLib.idle_add(self.process_ended, script.stem)
                GLib.idle_add(self.create_scripts_for_lnk_files, Path(wineprefix))
    ...
    def process_ended(self, script_stem):
        # Check if the process has ended and remove it from the running processes
        if script_stem in self.running_processes:
            # Remove the process from the running processes
            process_info = self.running_processes.pop(script_stem, None)

            if process_info:
                # Reset the play/stop button to the play state
                play_button = self.launch_button  # Assuming launch_button is the play/stop button
                ...
                # Remove the highlighted state from the row
                row = self.find_row_by_script_stem(script_stem)
                if row:
                    row.remove_css_class("highlighted")
        ...
        # Update the UI state now that the process has ended
        self.update_running_script_buttons()
