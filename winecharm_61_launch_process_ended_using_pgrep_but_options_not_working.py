...
#           icon_symlink_path.symlink_to(icon_path)
    ...
    def on_back_button_clicked(self, button):
        ...
        # Restore the open button to the top of the vbox if it's not already there
        ...
        # Reset the flowbox state and search mode
        self.search_button.set_active(False)
        self.search_entry.set_text("")

        # No need to explicitly show the main_frame or scrolled as they will be visible by default.
    ...
    def process_ended(self, script_stem):
        if script_stem in self.running_processes:
            ...
            if self.options_listbox:
                for option_row in self.options_listbox:
                    box = option_row.get_child()
                    for widget in box:
...
# NEW
    ...
    def replace_open_button_with_launch(self, script, row):
        ...
        launch_button.set_size_request(390, 36)
        launch_button.add_css_class("flat")
        launch_button.add_css_class("normal-font")

        launch_icon = Gtk.Image.new_from_icon_name("media-playback-start-symbolic")
        launch_label = Gtk.Label(label="Launch")
        launch_label.set_xalign(0)
        launch_label.set_hexpand(True)
        launch_label.set_ellipsize(Pango.EllipsizeMode.END)

        launch_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        launch_hbox.append(launch_icon)
        launch_hbox.append(launch_label)
        launch_button.set_child(launch_hbox)

        launch_button.connect("clicked", lambda btn: self.toggle_play_stop(script, launch_button, row))

        # Ensure the launch button is added and visible
        self.vbox.prepend(launch_button)
...
#NEW
    ...
    def launch_script(self, script, play_stop_button, row):
        ...
        try:
            ...
            # Store the row information for later use
            self.running_processes[script.stem] = {"row": row}
    ...
    def monitor_process(self, script_stem, exe_file):
        ...
        while True:
            try:
                ...
                if not pgrep_output:
                    # No processes are running, call process_ended
                    GLib.idle_add(self.process_ended, script_stem)
                    break

                print(pgrep_output)  # Debugging output to see the matched processes
    ...
    def process_ended(self, script_stem):
        if script_stem in self.running_processes:
            ...
            if process_info:
                ...
                # Update the button's functionality to relaunch the script
                self.launch_button.connect("clicked", lambda btn: self.launch_script(process_info["script"], self.launch_button, row))
    ...
    # This should prevent any further KeyError related to the removed buttons and clean up the process safely.
        ...
        if script.stem in self.running_processes:
            ...
            if process_info:
                ...
                try:
                    ...
                    # Check if options_button exists before resetting buttons
                    options_button = process_info.get("options_button")
                    if options_button:
                        self.reset_buttons(
                            process_info["play_button"],
                            process_info["play_icon"],
                            options_button,
                            process_info.get("gear_icon"),
                            script
                        )
