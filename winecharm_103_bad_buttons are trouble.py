...
#        self.create_script_list()
    ...
    def update_script_button_state(self, script_stem):
        ...
        if row:
            ...
            exe_file, _, _, _, _ = self.extract_yaml_info(Path(script_stem))
...
######## Refine
    ...
    def toggle_play_stop(self, script, play_stop_button, row):
        ...
        # Update both the button and row states after toggling
        self.update_script_button_state(script.stem)
    ...
    def terminate_script(self, script):
        ...
        if exe_name in self.running_processes:
            ...
            try:
                ...
                # Ensure the UI is updated after termination
                self.update_script_button_state(script.stem)
    ...
    def update_script_button_state(self, script_stem):
        # Find all scripts that match the exe_name
        matching_scripts = [script for script in self.find_python_scripts() if script.stem == script_stem]

        # Prioritize the one where script.stem and exe_name are the same
        preferred_script = None
        for script in matching_scripts:
            exe_file, _, _, _, _ = self.extract_yaml_info(script)
            exe_name = Path(exe_file).stem
            if script.stem == exe_name:
                preferred_script = script
                break

        # If no preferred script is found, just take the first match
        if not preferred_script and matching_scripts:
            preferred_script = matching_scripts[0]

        if preferred_script:
            exe_file, _, _, _, _ = self.extract_yaml_info(preferred_script)
            exe_name = Path(exe_file).name

            # Update the launch_button if it exists and matches the exe_name
            if self.launch_button and self.launch_button_exe_name == exe_name:
                if exe_name in self.running_processes:
                    play_icon = Gtk.Image.new_from_icon_name("media-playback-stop-symbolic")
                    ...
                    if self.selected_row:
                        self.selected_row.add_css_class("highlighted")
                else:
                    play_icon = Gtk.Image.new_from_icon_name("media-playback-start-symbolic")
                    self.launch_button.set_child(play_icon)
                    self.launch_button.set_tooltip_text("Play")
            ...
            print(f"Script file not found for stem: {script_stem}")
    ...
    def on_focus_in(self, controller):
        ...
        if self.launch_button_exe_name:
            print(f"[DEBUG] Checking if script is running for exe_name: {self.launch_button_exe_name}")

            # Cross-verify that the launch_button_exe_name corresponds to a running process
            if self.is_script_running(self.launch_button_exe_name):
                print(f"[DEBUG] Script is running for exe_name: {self.launch_button_exe_name}")
                
                launch_button, selected_row = self.exe_name_to_widgets.get(self.launch_button_exe_name, (None, None))

                if launch_button:
                    # Set the button to the "Stop" state
                    launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-stop-symbolic"))
                    launch_button.set_tooltip_text("Stop")

                # Ensure the row is highlighted if the script is running
                if selected_row:
                    selected_row.add_css_class("highlighted")
            else:
                print(f"[DEBUG] Script is not running for exe_name: {self.launch_button_exe_name}")
                
                # Reset the button to its initial launch state
                launch_button, selected_row = self.exe_name_to_widgets.get(self.launch_button_exe_name, (None, None))

                if launch_button:
                    launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-start-symbolic"))
                    launch_button.set_tooltip_text("Play")

                    # Reconnect the clicked event to launch the script
                    if hasattr(self, 'launch_button_handler_id'):
                        launch_button.disconnect(self.launch_button_handler_id)

                    self.selected_script = self.find_script_by_exe_name(self.launch_button_exe_name)
                    self.selected_row = selected_row

                    self.launch_button_handler_id = launch_button.connect(
                        "clicked", 
                        lambda btn: self.toggle_play_stop(self.selected_script, launch_button, selected_row)
                    )

                self.launch_button_exe_name = None  # Clear the exe_name

                # Remove highlighting from the row
                if selected_row:
                    selected_row.remove_css_class("highlighted")
        else:
            print("[DEBUG] launch_button_exe_name is None or not found, resetting button to Play state")

            # If exe_name is None, reset the button to Play state
            launch_button, selected_row = self.exe_name_to_widgets.get(self.launch_button_exe_name, (None, None))
    ...
    def replace_open_button_with_launch(self, script, row):
        ...
        # Assign exe_name to the global attribute
        self.launch_button_exe_name = exe_name
    ...
    def on_focus_in(self, controller):
        ...
        # Check and update the state of the launch button and row
        if self.launch_button_exe_name:
            print(f"[DEBUG] Checking if script is running for exe_name: {self.launch_button_exe_name}")

            if self.is_script_running(self.launch_button_exe_name):
                print(f"[DEBUG] Script is running for exe_name: {self.launch_button_exe_name}")
                # Ensure that the launch button is not None before setting its properties
                if self.launch_button:
                    self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-stop-symbolic"))
                    self.launch_button.set_tooltip_text("Stop")
                ...
                print(f"[DEBUG] Script is not running for exe_name: {self.launch_button_exe_name}")
                # Reset the button to play and remove highlight from the row
                if self.launch_button:
                    self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-start-symbolic"))
                    self.launch_button.set_tooltip_text("Play")
    ...
    def is_script_running(self, exe_name):
        # Check if exe_name exists in the running_processes dictionary and ensure the process is still active
        if exe_name in self.running_processes:
            process_info = self.running_processes[exe_name]
            pid = process_info.get("pid")
            if pid:
                try:
                    ...
                    os.kill(pid, 0)  # Sending signal 0 does not kill the process, it just checks if it's running
                    return True
                except OSError:
        ...
        if exe_name in self.running_processes:
            ...
            try:
                if pid:
                    ...
                    os.kill(pid, 0)  # Signal 0 doesn't kill the process; it just checks if it's alive
                    os.killpg(os.getpgid(pid), signal.SIGTERM)
                    print(f"Terminated script {script} with PID {pid}")
                del self.running_processes[exe_name]
            except ProcessLookupError:
                print(f"No such process with PID {pid}, process may have already exited.")
            except Exception as e:
                print(f"Error terminating script {script}: {e}")
            finally:
                # Ensure the UI is updated after termination
                self.update_script_button_state(exe_name)
