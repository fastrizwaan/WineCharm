...
###################### chatgpt4 ok
    def on_open(self, app, files, *args):
        ...
        if self.single_prefix:
            ...
            if self.arch == 'win32':
                arch_templates = [(True, self.default_template_win32, 'win32', self.single_prefix_dir_win32)]
            else:
                arch_templates = [(True, self.default_template_win64, 'win64', self.single_prefix_dir_win64)]
        ...
        def process_file():
            if self.command_line_file:
                ...
                if file_extension in ['.exe', '.msi']:
                    ...
                    GLib.idle_add(self.show_processing_spinner, "Processing File")
                    GLib.idle_add(self.process_cli_file_later, self.command_line_file)
                elif file_extension in ['.wzt', '.bottle', '.prefix']:
                    print("B"*100)
                    GLib.idle_add(self.restore_prefix_bottle_wzt_tar_zst, self.command_line_file)
                else:
                    print("C"*100)
                    print(f"Invalid file type: def on_open: {file_extension}. Only .exe and .msi files are allowed.")
                    GLib.timeout_add_seconds(0.5, self.show_info_dialog, "Invalid File Type", "Only .exe and .msi files are supported.")
                    self.command_line_file = None
                    return False
        
        if needs_initialization:
            print("Waiting for template initialization to complete before processing file...")
            GLib.idle_add(lambda: self.on_template_initialized() or process_file())
        else:
            self.create_script_list()
            self.set_dynamic_variables()
            process_file()
...
################# deepseek
    def on_open(self, app, files, *args):
        ...
        if self.single_prefix:
            ...
            current_arch = self.arch if hasattr(self, 'arch') else 'win64'
            template = self.default_template_win32 if current_arch == 'win32' else self.default_template_win64
            arch_templates = [(True, template, current_arch, None)]
        ...
        if not needs_initialization:
            ...
            if self.command_line_file:
                file_path = Path(self.command_line_file)
                if not file_path.exists():
                    print(f"File not found: {self.command_line_file}")
                    self.command_line_file = None
                    return
                ...
                if file_extension in ['.exe', '.msi']:
                    ...
                    GLib.idle_add(self.show_info_dialog, "Invalid File Type", "Supported: .exe, .msi, .wzt, .bottle, .prefix")
                    self.command_line_file = None
    ...
    def on_template_initialized(self, arch=None):
        self.print_method_name()
        print(f"Template initialization complete for {arch}.")
        self.initializing_template = False
        self.hide_processing_spinner()
        self.set_open_button_label("Open")
        self.search_button.set_sensitive(True)
        ...
        if self.command_line_file:
            ...
            file_extension = file_path.suffix.lower()
            if file_extension in ['.exe', '.msi']:
                GLib.idle_add(self.show_processing_spinner, "Launching...")
                self.process_cli_file_later(self.command_line_file)
            elif file_extension in ['.wzt', '.bottle', '.prefix']:
                self.restore_prefix_bottle_wzt_tar_zst(self.command_line_file)
                self.command_line_file = None  # Reset after processing
    ...
    def process_cli_file_in_thread(self, file_path):
        ...
        GLib.idle_add(lambda: self.show_processing_spinner("Processing..."))
        try:
            steps = [("Creating configuration", lambda: GLib.timeout_add(500, lambda: self.create_yaml_file(str(file_path), None)))]
            self.total_steps = len(steps)
            ...
            print(f"Error: {e}")
            GLib.idle_add(lambda: self.show_info_dialog("Error", str(e)))
        finally:
            def cleanup():
                self.hide_processing_spinner()
                self.create_script_list()
...
################################ retusing
    def on_open(self, *args):
        ...
        if not hasattr(self, 'initialized'):
            ...
            self.template_init_in_progress = False
            self.initialize_app()
        GLib.idle_add(self.window.present)

    def ixnitialize_app(self):
        self.print_method_name()
        print("--------------- INITIALIZE_APP ------------------")
        ...
        if not hasattr(self, 'window') or not self.window:
            ...
            # Only start template initialization if it's not already in progress
            if not self.template_init_in_progress:
                self.template_init_in_progress = True
                self.initialize_templates()

    def initialize_templates(self):
        """Handle template initialization sequence"""
        self.print_method_name()
        ...
        # Determine which templates need initialization
        arch_templates = []
        if self.single_prefix:
            arch_templates = [
                (True, self.default_template_win32, 'win32', self.single_prefix_dir_win32),
        ...
        self.pending_templates = arch_templates.copy()
        self.process_next_template()


    def handle_template_initialized(self, arch):
        """Handle completion of template initialization with delay"""
        self.print_method_name()
        self.on_template_initialized(arch)
        
        # Continue with next template if any
        if self.pending_templates:
            self.process_next_template()
        else:
            print("All templates initialized...")
            # Set a flag to indicate we're in the cooling-off period
            self.template_cooling_down = True
            GLib.timeout_add_seconds(1, self.complete_initialization)
    ...
    def process_next_template(self):
        ...
        if not self.pending_templates:
            ...
            # Ensure CLI file is processed
            if hasattr(self, 'command_line_file') and self.command_line_file:
                GLib.idle_add(self.process_cli_file_later, self.command_line_file)
            return

        check, template, arch, single_dir = self.pending_templates.pop(0)
        if check and not template.exists():
            print(f"Initializing {arch} template...")
            self.initialize_template(
                template,
                lambda a=arch: self.handle_template_initialized(a),
                arch=arch
            )
        else:
            self.process_next_template()
    ...
    def process_next_template(self):
        ...
        if not self.pending_templates:
            ...
            self.set_dynamic_variables()
            if hasattr(self, 'command_line_file') and self.command_line_file:
                GLib.timeout_add_seconds(1, self.process_cli_file_later, self.command_line_file)
            return

        check, template, arch, _ = self.pending_templates.pop(0)
        if check and not template.exists():
            print(f"Initializing {arch} template...")
            self.initialize_template(template,
                                    lambda a=arch: self.handle_template_initialized(a),
                                    arch=arch)
        else:
            self.process_next_template()
    ...
    def process_next_template(self):
        ...
        if not self.pending_templates:
            ...
            # Process CLI file after slight delay to ensure UI is ready
            if hasattr(self, 'command_line_file') and self.command_line_file:
                GLib.idle_add(self.process_cli_file_later, self.command_line_file)
                self.command_line_file = None  # Prevent duplicate processing
            return
...
############################
    def on_open(self, *args):
        ...
        # Always present window and check CLI file
        GLib.idle_add(self.window.present)
        GLib.idle_add(self.try_process_cli_file)
    ...
    def initialize_templates(self):
        ...
        # Single template check based on arch
        target_template = (
            self.default_template_win32 if self.arch == 'win32'
            else self.default_template_win64
        )
        
        if not target_template.exists():
            print(f"Initializing {self.arch} template...")
            self.template_init_in_progress = True
            self.initialize_template(
                target_template,
                lambda: self.on_template_initialized(self.arch),
                arch=self.arch
            )
        else:
            print("Template already exists - marking initialized")
    ...
    def complete_initialization(self):
        ...
        # Explicitly trigger CLI processing after initialization
        GLib.idle_add(self.try_process_cli_file)

    def try_process_cli_file(self):
        """Process CLI file if app is initialized and file exists"""
        if self.initialized and hasattr(self, 'command_line_file') and self.command_line_file:
            print(f"Processing CLI file: {self.command_line_file}")
            self.process_cli_file_later(self.command_line_file)
           # self.command_line_file = None  # Clear only AFTER processing
    ...
    def on_open(self, *args):
        ...
        # Always present window and check CLI file
        GLib.idle_add(self.window.present)
        if self.initialized and self.command_line_file:
            print(f"Processing CLI file: {self.command_line_file}")
            self.process_cli_file_later(self.command_line_file)
