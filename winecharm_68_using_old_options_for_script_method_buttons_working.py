...
#           icon_symlink_path.symlink_to(icon_path)
    ...
    def callback_wrapper(self, callback, script, button=None, *args):
        if "button" in callback.__code__.co_varnames:
            return callback(script, button, *args)
        else:
    ...
    def show_options_for_script(self, script, row):
        ...
        options = [
            ...
            ("Wine Arguments", "preferences-system-symbolic", self.show_wine_arguments)
        ]
...
# NEW NEW NEW
    ...
    def show_wine_arguments(self, script, button):
        self.replace_button_with_entry_overlay(script, "Wine Arguments", button)
    ...
    def replace_button_with_overlay(self, script, confirmation_text, action_type, button):
        ...
        if isinstance(parent, Gtk.FlowBoxChild):
            # Extract the icon and label from the original button
            button_hbox = button.get_child()
            original_icon = button_hbox.get_first_child()
            original_label = original_icon.get_next_sibling()
            ...
            # Reuse the original icon and label
            if original_icon:
                confirmation_box.append(original_icon)
            if original_label:
                confirmation_box.append(original_label)
    ...
    def replace_button_with_entry_overlay(self, script, prompt_text, button):
        parent = button.get_parent()
