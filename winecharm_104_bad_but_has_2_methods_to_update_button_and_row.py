...
class WineCharmApp(Gtk.Application):
    def __init__(self):
        ...
        self.current_launch_button_script = None
...
#        self.create_script_list()
        ...
        for exe_name, process_info in list(self.running_processes.items()):
            if exe_name not in current_running_processes:
                ...
                if self.launch_button:
                    self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-stop-symbolic"))
                    self.launch_button.set_tooltip_text("Stop cleanup_ended_processes")
                del self.running_processes[exe_name]
            else:
                ...
                if self.launch_button:
                    ...
                    self.launch_button.set_tooltip_text("Play cleanup_ended_processes")
                del self.running_processes[exe_name]
    ...
    def show_options_for_script(self, script, row):
        ...
        # Set the current script/exe_name
        exe_file, _, _, _, _ = self.extract_yaml_info(script)
        self.current_launch_button_script = Path(exe_file).name
...
######## Refine
    ...
    def update_launch_button_state(self, script, row):
        print("666666666666")
        print(self.current_launch_button_script == Path(script).name)
        
        if self.launch_button and self.current_launch_button_script == Path(script).name:
            print("77777777777777")
            self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-start-symbolic"))
            self.launch_button.set_tooltip_text("Play")

            # Disconnect the previous handler if it exists
            if hasattr(self, 'launch_button_handler_id') and self.launch_button_handler_id:
                self.launch_button.disconnect(self.launch_button_handler_id)
                print("8888888")
            # Connect the launch button to launch_script and store the handler ID
            self.launch_button_handler_id = self.launch_button.connect(
                "clicked", lambda btn: self.launch_script(script, self.launch_button, row)
            )

    def update_row_highlight(self, row, highlight=True):
        if row:
            if highlight:
                row.add_css_class("highlighted")
            else:
...
#def some_other_method(self, script, row):
#self.update_row_highlight(row, highlight=True)
#self.update_launch_button_state(script, row)


    def show_options_for_script(self, script, row):
        self.main_frame.set_child(Gtk.Label(label=f"Options for {script.stem}"))
        self.replace_open_button_with_launch(script, row)
        self.update_script_button_state(script)  # Pass the full script path
        # Set the current script/exe_name
        exe_file, _, _, _, _ = self.extract_yaml_info(script)
        self.current_launch_button_script = Path(exe_file).name
...
###################################################################################################
    def cleanup_ended_processes(self, current_running_processes):
        ...
        for exe_name, process_info in list(self.running_processes.items()):
            ...
            if exe_name not in current_running_processes:
                print("22222222")
                self.update_row_highlight(row, highlight=False)
                self.update_launch_button_state(script, row)
                del self.running_processes[exe_name]
            else:
                print("33333333333333")
    ...
    def process_ended(self, exe_name):
        ...
        if process_info:
            ...
            # Unhighlight the row since the process has ended
            self.update_row_highlight(row, highlight=False)

            # Update the launch button state
            self.update_launch_button_state(script, row)

            # Process any remaining tasks after the process has ended
            script_path = process_info.get("script")
            if script_path and script_path.exists():
                wineprefix = self.extract_yaml_info(script_path)[1]
                if wineprefix:
            ...
            # Remove the process from the running_processes dictionary
            if exe_name in self.running_processes:
                self.update_row_highlight(row, highlight=False)
                # Update the launch button state
                self.update_launch_button_state(script, row)
        ...
        if exe_name in self.running_processes:
            ...
            try:
                if pid:
                    ...
                    print(f"No PID found for {exe_name}, removing from running processes")

            except ProcessLookupError:
                print(f"Process {pid} for script {script} already terminated")

            except Exception as e:
                print(f"Error terminating script {script}: {e}")

            finally:
                # Update UI and remove the process from running processes list
                self.update_row_highlight(row, highlight=False)
                self.update_launch_button_state(script, row)
                del self.running_processes[exe_name]
