...
class WineCharmApp(Gtk.Application):
    def __init__(self):
        ...
        self.template_cooling_down = False
...
######################## on open fix?
    ...
    def process_next_template(self):
        ...
        if check:
            if not template.exists():
                print(f"Initializing {arch} template...")
                self.initialize_template(template, lambda a=arch: self.handle_template_initialized(a), arch=arch)
            #elif self.single_prefix and single_dir and not single_dir.exists():
                #print(f"Copying {arch} template to single prefix...")
                #self.process_next_template()
                # self.copy_template(single_dir)
            else:
    ...
    def handle_template_initialized(self, arch):
        ...
        if self.pending_templates:
            ...
            print("All templates initialized, waiting 15 seconds for Wine initialization...")
            # Set a flag to indicate we're in the cooling-off period
            self.template_cooling_down = True
            # Wait 15 seconds before marking initialization as fully complete
