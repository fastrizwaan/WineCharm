...
def handle_signal(signum, frame):
    ...
    if signum == signal.SIGINT:
        print("Caught SIGINT (Ctrl+C)")
    elif signum == signal.SIGTERM:
        print("Caught SIGTERM (Termination)")
    elif signum == signal.SIGHUP:
        print("Caught SIGHUP (Hangup)")
    elif signum == signal.SIGQUIT:
        print("Caught SIGQUIT (Quit)")
    else:
        print(f"Caught signal: {signum}")

    sys.exit(0)

# Register handlers for all major signals
...
#self.window.present()
    ...
    def handle_sighup(self, signum, frame):
        ...
        # Decide whether to ignore or handle this signal
        self.cleanup_and_exit()
    ...
    def get_running_processes(self):
        ...
        try:
            ...
            for script in self.find_python_scripts():
                ...
                script_key = yaml_info['sha256sum']
                exe_name = Path(yaml_info['exe_file']).name
                exe_full_path = str(Path(yaml_info['exe_file']).resolve())

                # Improved matching: Check if the exe_full_path is in the command
                matching_processes = [
                    (int(line.split()[0]), line.split(None, 1)[1]) for line in pgrep_output
                    if exe_full_path in line or exe_name in line
                ]

                print(f"Matching processes for {script.stem}: {matching_processes}")  # Debugging output

                if matching_processes:
                    for pid, cmd in matching_processes:
                        # Debug to check the matched command
                        print(f"Process matched for {script.stem}: PID={pid}, CMD={cmd}")

                        row = self.script_buttons.get(script_key)
                        if row:
                            print(f"Highlighting row for {script.stem}")  # Debugging output
...
#-------------------------------
    ...
    def terminate_script(self, script_key):
        ...
        # Ensure winedbg is used to refine the list of processes
        refined_processes = self.get_wine_processes(wineprefix, exe_name)
        
        for refined_pid, refined_exe_name in refined_processes:
            if refined_exe_name == exe_name:
                try:
                    print(f"Attempting to terminate PID: {refined_pid}")
                    os.kill(refined_pid, signal.SIGTERM)
                    print(f"Terminated process {script_key} with PID: {refined_pid}")
                except ProcessLookupError:
                    print(f"Process with PID {refined_pid} not found. It may have already terminated.")
                except Exception as e:
                    print(f"Failed to terminate process {refined_pid}: {e}")
        
        # Remove the script from the running_processes dictionary
        del self.running_processes[script_key]
        print(f"Removed script_key: {script_key} from running processes.")
...
################### experiment with order
    def get_wine_processes(self, wineprefix, exe_name):
        try:
            command = f"WINEPREFIX={shlex.quote(wineprefix)} winedbg --command 'info proc' grep {exe_name}"
    ...
    def get_running_processes(self):
        ...
        try:
            ...
            for script in self.find_python_scripts():
                ...
                script_key = yaml_info['sha256sum']
                exe_name = Path(yaml_info['exe_file']).name
                wineprefix = str(Path(yaml_info['wineprefix']).resolve())
               ...
               # print(f"Initial matching processes for {script.stem}: {matching_processes}")  # Debugging output

                # If duplicates are found, refine using winedbg
                if len(matching_processes) > 1:
                    wine_processes = self.get_wine_processes(wineprefix, exe_name)  # Note the use of self.get_wine_processes
                    print(f"Refined processes using winedbg for {script.stem}: {wine_processes}")  # Debugging output
                    matching_processes = wine_processes
...
#####
    def get_running_processes(self):
        ...
        try:
            ...
            for script in self.find_python_scripts():
                ...
                script_key = yaml_info['sha256sum']
                exe_name = Path(yaml_info['exe_file']).name
                wineprefix = str(Path(script).parent)
                ...
                if matching_processes:
                    for pid, cmd in matching_processes:
                        ...
                        if row:
                            ...
                            self.update_ui_for_running_process(script_key, row, current_running_processes)
                            self.running_processes[script_key] = current_running_processes[script_key]  # Ensure running_processes is updated correctly
                else:
                    self.process_ended(script_key)
    ...
    def terminate_script(self, script_key):
        ...
        for refined_pid, refined_exe_name in refined_processes:
            if refined_exe_name == exe_name and refined_pid == pid:  # Match both exe_name and pid
                try:
                    print(f"Attempting to terminate PID: {refined_pid}")
                    os.kill(refined_pid, signal.SIGTERM)
                    print(f"Terminated process {script_key} with PID: {refined_pid}")
...
################################
    ...
    def terminate_script(self, script_key):
       ...
       #     return
        ...
        refined_processes = self.get_wine_processes(wineprefix, exe_name)
        print("=============================================================")
        print(refined_processes)
        for refined_pid, refined_exe_name in refined_processes:
            if refined_exe_name == exe_name and refined_pid == pid:  # Ensure both exe_name and pid match
                try:
                    print(f"Attempting to terminate PID: {refined_pid}")
                    os.kill(refined_pid, signal.SIGKILL)
                    print(f"Terminated process {script_key} with PID: {refined_pid}")
                except ProcessLookupError:
                    print(f"Process with PID {refined_pid} not found. It may have already terminated.")
                except PermissionError:
                    print(f"Permission denied to terminate process with PID: {refined_pid}")
                except Exception as e:
                    print(f"Failed to terminate process {refined_pid}: {e}")
...
####################################
    ...
    def toggle_play_stop(self, script, play_stop_button, row):
        yaml_info = self.extract_yaml_info(script)
        script_key = yaml_info['sha256sum']  # Ensure you're using the correct key
    ...
    def get_wine_processes(self, wineprefix, exe_name):
        try:
            wineprefix_str = str(wineprefix)  # Convert PosixPath to string
            command = f"WINEPREFIX={shlex.quote(wineprefix_str)} winedbg --command 'info proc' |grep {exe_name}"
            winedbg_output = subprocess.check_output(command, shell=True).decode().splitlines()
            print("=======================")
            print(winedbg_output)
            processes = []
            for line in winedbg_output:
                # Print each line for debugging purposes
                #print(f"winedbg output line: {line}")
...
def get_wine_processes(self, wineprefix, exe_name, callback):
    def run_winedbg():
        try:
            wineprefix_str = str(wineprefix)  # Convert PosixPath to string
            command = f"WINEPREFIX={shlex.quote(wineprefix_str)} winedbg --command 'info proc'"
            winedbg_output = subprocess.check_output(command, shell=True).decode().splitlines()
            ...
            # Use GLib.idle_add to update the UI on the main thread
            GLib.idle_add(callback, processes)
        except subprocess.CalledProcessError as e:
            print(f"Error running winedbg: {e}")
            GLib.idle_add(callback, [])

    # Start the winedbg command in a separate thread
    threading.Thread(target=run_winedbg).start()
####################
    def open_terminal(self, script, *args):
        wineprefix = Path(script).parent
        print(f"Opening terminal for {wineprefix}")
        ...
        if shutil.which("flatpak-spawn"):
            command = [
                "setsid",  # Start a new session to detach from WineCharm
                "nohup",  # Ensure the process ignores hangups (SIGHUP)
                "flatpak-spawn",
                "--host",
                "gnome-terminal",
                "--",
        ...
        try:
            # Redirect stdout, stderr, and stdin to /dev/null to completely detach the terminal
            with open('/dev/null', 'w') as devnull:
                subprocess.Popen(command, stdout=devnull, stderr=devnull, stdin=devnull)
        except Exception as e:
            print(f"Error opening terminal: {e}")
##############################
    def open_terminal(self, script, *args):
        ...
        if shutil.which("flatpak-spawn"):
            command = [
                "setsid",  # Start the process in a new session to detach it from WineCharm
                "flatpak-spawn",
                "--host",
                "gnome-terminal",
                "--wait",
                ...
                "bash",
                "-c",
                rf'trap "" SIGINT SIGTERM SIGHUP; export PS1="[\u@\h:\w]\\$ "; export WINEPREFIX={shlex.quote(str(wineprefix))}; cd {shlex.quote(str(wineprefix))}; exec bash --norc -i'
            ]
        else:
            command = [
                "setsid",
...
######################### launch_ in bash and terminate using shell
    def launch_script(self, script, play_stop_button, row):
        yaml_info = self.extract_yaml_info(script)
        exe_file = yaml_info['exe_file']
        wineprefix = Path(script).parent
        ...
        # Launch the command inside a bash subshell
        command = (
            f"cd {shlex.quote(str(Path(exe_file).parent))} && "
            f"WINEPREFIX={shlex.quote(str(wineprefix))} "
            f"bash -c '{shlex.quote(runner)} {shlex.quote(str(exe_name))} {script_args}'"
        )
        print(command)
        
        try:
            # Use os.setsid to start a new process group so we can manage the entire group
            process = subprocess.Popen(
                command,
                shell=True,
                preexec_fn=os.setsid,
    ...
    def terminate_script(self, script):
        ...
        if script_key in self.running_processes:
            ...
            try:
                if pid:
                    print(f"Attempting to terminate process group with PGID: {os.getpgid(pid)}")
                    os.killpg(os.getpgid(pid), signal.SIGTERM)
                    print(f"Terminated process group for script {script_key} with PGID: {os.getpgid(pid)}")
