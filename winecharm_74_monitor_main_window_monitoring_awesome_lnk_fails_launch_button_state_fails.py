...
class WineCharmApp(Gtk.Application):
    ...
    def check_running_processes_and_update_buttons(self):
        ...
        try:
            ...
            running_processes_output = subprocess.check_output(["pgrep", "-af", r"\.exe"]).decode().splitlines()
                ...
                try:
                    ...
                    if is_running:
                        ...
                        if script.stem not in self.running_processes:
                            ...
                            self.running_processes[script.stem] = {"row": row, "script": script, "exe": exe_name}
                    else:
...
#           icon_symlink_path.symlink_to(icon_path)
    ...
    def on_kill_all_clicked(self, action=None, param=None):
        ...
        try:
            ...
            winecharm_pid_output = subprocess.check_output(["pgrep", "-af", do_not_kill]).decode()
            ...
            try:
                ...
                wine_exe_output = subprocess.check_output(["pgrep", "-af", r"\.exe"]).decode()
    ...
    def monitor_process(self, script_stem, exe_file):
        ...
        while self.monitoring_active:
            try:
                ...
                pgrep_output = subprocess.check_output(["pgrep", "-af", exe_name_upto_fifteen_chars]).decode()
                print(exe_name_upto_fifteen_chars)
...
##NEW
    ...
    def monitor_process(self, script_stem, exe_file):
        ...
        while True:
            if not self.monitoring_active:
                # If monitoring is inactive, pause the monitoring loop
                #print(f"Monitoring paused for: {exe_name_upto_fifteen_chars}")
                time.sleep(3)  # Wait before checking the monitoring status again
                continue
...
###########################
    def check_running_processes_and_update_buttons(self):
        ...
        try:
            running_processes_output = subprocess.check_output(["pgrep", "-af", r"\.exe"]).decode().splitlines()
            #print("Running processes output:", running_processes_output)  # Debugging line
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    script_key = hashlib.sha256(str(script).encode()).hexdigest()
                    exe_name_pattern = re.compile(r'\b' + re.escape(exe_name) + r'\b')

                    relevant_processes = [
                        process for process in running_processes_output
                        if exe_name_pattern.search(process)
                    ]

                    if relevant_processes:
                        pid = int(relevant_processes[0].split()[0])
                        row = self.find_row_by_script_stem(script.stem)  # Ensure this returns the correct row

                        if script_key not in self.running_processes:
                            self.running_processes[script_key] = {
                                "row": row,  # Correctly reference the row here
                                "script": script,
                                "exe_name": exe_name,
                                "pid": pid
                            }
                            print(f"Script running: {script_key} : {exe_name}, PID: {pid}")  # Debugging line

                    self.toggle_running_button_highlight(script_key, self.running_processes.get(script_key, {}).get("row"))
    ...
    def toggle_running_button_highlight(self, script_key, row):
        print("========================= HELLO ================")
        if row:
            ...
            if process_info:
                ...
                if exe_name and pid:
                    try:
                        ps_output = subprocess.check_output(["ps", "-p", str(pid), "-o", "comm="]).decode().strip()
                        if ps_output == exe_name:
                            row.add_css_class("highlighted")
                        else:
                            row.remove_css_class("highlighted")
                            self.running_processes.pop(script_key)
                    except subprocess.CalledProcessError:
                        row.remove_css_class("highlighted")
                        self.running_processes.pop(script_key)
                else:
            ...
            print(f"No row found for script_key: {script_key}")


    def create_script_list(self):
        for script in self.scripts:
            button = Gtk.Button(label=script.name)
            self.script_rows.append(button)
            # Add button to the UI container

    def find_row_by_script_stem(self, script_stem):
        for row in self.script_rows:
            label = row.get_label().strip().replace(" ", "_")
            if label == script_stem:
                return row
        return None

    def check_running_processes_and_update_buttons(self):
        # Your existing code that checks running processes

        for script_key, process_info in self.running_processes.items():
            row = self.find_row_by_script_stem(script_key)
            if row:
                self.highlight_button(row)  # or whatever method you use to highlight the button
            else:
                print(f"No row found for script_key: {script_key}")
    ...
    def create_script_list(self):
        print("Creating script list...")
        self.flowbox.remove_all()  # Clear the existing list

        scripts = self.find_python_scripts()  # Find all the .charm files in the prefixes_dir
        ...
        for script in scripts:
            ...
            row = self.create_script_row(script)  # Create a row for each script
            self.flowbox.append(row)  # Add the row to the flowbox
            button = Gtk.Button(label=script.name)
            self.script_rows.append(button)
            self.toggle_running_button_highlight(script.stem, row)  # Highlight the row if the script is running
    ...
    def create_script_list(self):
        ...
        for script in self.scripts:
            print(f"Processing script: {script}")
            # Add code to create UI elements for each script
            button = Gtk.Button(label=script.stem)  # Example button creation
            self.script_rows.append(button)  # Assuming self.script_rows is a container
            # Add button to the UI
            self.main_container.add(button)
    ...
    def create_script_list(self):
        ...
        for script in scripts:
            ...
            if script_key in self.running_processes:
                self.toggle_running_button_highlight(script_key, row)  # Highlight the row if the script is running
            else:
                self.toggle_running_button_highlight(script.stem, row)
       ...
       # print("Creating script list...")
        ...
        for script in scripts:
           # print(f"Processing script: {script}")
            ...
            # Generate the script key
            script_key = hashlib.sha256(str(script).encode()).hexdigest()
            
            # Check if the script is running and highlight if necessary
            if script_key in self.running_processes:
                print(f"Highlighting running script: {script_key}")
                self.toggle_running_button_highlight(script_key, row)
            else:
               # print(f"Not highlighting script: {script_key}")
                self.toggle_running_button_highlight(script.stem, row)

        # No need for self.flowbox.show_all()
    def check_running_processes_and_update_buttons(self):
        if not self.monitoring_active:
            self.stop_monitoring()
            return False  # Stop the loop if not active
        ...
        try:
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    if relevant_processes:
                        ...
                        current_running_processes[script_key] = self.running_processes[script_key]
                        self.toggle_running_button_highlight(script_key, row)
                    else:
                        # If the process is not running, ensure it's not highlighted
                        if script_key in self.running_processes:
                            print(f"Process ended for script: {script.stem}")
                            row = self.running_processes[script_key]["row"]
                            if row:
                                row.remove_css_class("highlighted")
            ...
            # Clean up the running_processes dictionary to remove entries no longer running
    ...
    def launch_script(self, script, play_stop_button, row):
        ...
        command = f"cd {shlex.quote(str(exe_dir))} && WINEPREFIX={shlex.quote(str(wineprefix))} wine {shlex.quote(str(exe_name))} {script_args}"
        ...
        try:
            ...
            # Store the row, script information, and PID for later use
            script_key = hashlib.sha256(str(script).encode()).hexdigest()
            self.running_processes[script_key] = {"row": row, "script": script, "exe_name": exe_name, "pid": process.pid}
    ...
    def toggle_play_stop(self, script, play_stop_button, row):
        ...
        if script_key in self.running_processes:
            ...
            # If the script is not running, launch it
            self.launch_script(script, play_stop_button, row)
    ...
    def terminate_script(self, script):
        ...
        if script_key in self.running_processes:
            ...
            if process_info:
                ...
                if pid:
                    try:
                        # Get all child processes related to the main PID
                        pgrep_output = subprocess.check_output(["pgrep", "-P", str(pid)]).decode().splitlines()
                        child_pids = [int(pid) for pid in pgrep_output]

                        # Terminate the child processes
                        for child_pid in child_pids:
                            try:
                                os.kill(child_pid, signal.SIGKILL)
                                print(f"Terminated child process with PID: {child_pid}")
                            except ProcessLookupError:
                                print(f"Child process with PID {child_pid} already terminated.")

                        # Terminate the main process
                        os.kill(pid, signal.SIGKILL)
                        print(f"Terminated main process with PID: {pid}")

                        # Update the button state after termination
                        row = process_info.get("row")
                        if row:
                            play_icon = Gtk.Image.new_from_icon_name("media-playback-start-symbolic")
                            play_stop_button = row.get_first_child()
                    ...
                    print(f"No PID found for script: {script}")
            else:
                print(f"No running process info found for script: {script}")
        else:
            print(f"No running process found for {script}")
    ...
    def check_running_processes_and_update_buttons(self):
        ...
        try:
            running_processes_output = subprocess.check_output(["pgrep", "-af", r"\.exe"]).decode().splitlines()
            current_running_processes = {}  # Dictionary to track currently running processes
                    ...
                    if relevant_processes:
                        ...
                        if row is not None:
                            ...
                            if script_key not in self.running_processes:
                                ...
                                print(f"Script running: {script_key} : {exe_name}, PID: {pid}")

                            # Highlight the row
                            row.add_css_class("highlighted")
                        else:
                            print(f"No row found for script: {script.stem}")  # Debugging line

                    else:
                        # If the process has ended, unhighlight the row
                        if script_key in self.running_processes:
                            print(f"Process ended for script: {script_key} : {exe_name}")
                            row = self.find_row_by_script_stem(script.stem)
                            if row is not None:
                                row.remove_css_class("highlighted")
                            self.running_processes.pop(script_key)
            ...
            # Update self.running_processes to match current running processes
            self.running_processes = current_running_processes
    ...
    def toggle_running_button_highlight(self, script_name, row):
        if row:
            process_info = self.running_processes.get(script_name)
            if process_info:
                exe_name = process_info.get("exe_name")
                pid = process_info.get("pid")
                ...
                if exe_name and pid:
                    try:
                        ...
                        if ps_output == exe_name:
                            ...
                            row.remove_css_class("highlighted")
                            self.running_processes.pop(script_name)
                    except subprocess.CalledProcessError:
                        row.remove_css_class("highlighted")
                        self.running_processes.pop(script_name)
                else:
            ...
            print(f"No row found for script_name: {script_name}")

    def find_row_by_script_stem(self, script_stem):
        # Convert underscores to spaces to match the button label
        script_label = script_stem.replace('_', ' ')
        for child in self.flowbox:
            button = child.get_child()  # Get the button inside the FlowBox child
            if button.get_label() == script_label:
                return child
        return None
    ...
    def toggle_running_button_highlight(self, script_name, row):
        if row:
            ...
            print(f"No row found for script_name: {script_name.replace('_', ' ')}")

    def find_row_by_script_stem(self, script_stem):
        # Convert underscores to spaces to match the button label
        script_label = script_stem.replace('_', ' ')
        print(f"Searching for script with label: {script_label}")
        for child in self.flowbox.get_children():
            button = child.get_child()  # Get the button inside the FlowBox child
            button_label = button.get_label()
            print(f"Checking button with label: {button_label}")
            if button_label == script_label:
                print(f"Found matching row for script: {script_stem}")
                return child
        print(f"No matching row found for script: {script_stem}")
        return None
    ...
    def create_script_list(self):
        ...
        self.flowbox.show()  # Show the updated flowbox
    ...
    def find_row_by_script_stem(self, script_stem):
        ...
        def match_button(row):
            button = row.get_first_child()  # Assuming button is the first child in the row
            if button and isinstance(button, Gtk.Button):
                button_label = button.get_label()
                print(f"Checking button with label: {button_label}")
                if button_label == script_label:
                    print(f"Found matching row for script: {script_stem}")
        ...
        matching_row = None
        self.flowbox.foreach(lambda row: match_button(row) if matching_row is None else None)
    ...
    def create_script_list(self):
        #print("Creating script list...")
        self.flowbox.remove_all()  # Clear the existing list
        self.script_buttons = {}  # Initialize or reset the dictionary to store button references
        ...
        for script in scripts:
            ...
            if row is not None:
                self.flowbox.append(row)  # Add the row to the flowbox
                script_stem = script.stem.replace("_", " ")
                self.script_buttons[script_stem] = row  # Store the button reference
                self.toggle_running_button_highlight(script.stem, row)  # Highlight the row if the script is running
...
#### using 
        #``
        #======= Running Processes =========
        #{'a28a4a2322bff4d988e06073f3bcefdd197eba6826a110554b1dbb3f2b004d4b': {'row': <Gtk.Button object at #0x7f7b8af33e80 (GtkButton at 0x3db289f0)>, 's#cript': PosixPath('/home/asif/.var/app/#io.github.fastrizwaan.WineCharm/data/winechar#m/Prefixes/iexplore-71d214105c/Wine.charm'), #'exe_name': 'iexplore.exe', 'pid': 158856}, ##io.github.fastrizwaan.WineCharm/data/winecharm/#Prefixes/Popcorn-Time-0.5.1-win64-0.44.5-#Setup-1276272082/Popcorn-Time-0.5.1-win64-0.44.5-#Setup.charm'), 'exe_name': 'Popcorn-Time-0.5.1-#win64-0.44.5-Setup.exe', 'pid': 158991}, #'212662cf1e0cd6f9661764dfe9520b7d7b702fbab67759f5e825992df31a991d': {'row': <Gtk.Button object at #0x7f7b801ee8c0 (GtkButton at 0x3dc328f0)>, 'script': PosixPath('/home/asif/.var/app/##io.github.fastrizwaan.WineCharm/data/winecharm/Prefixes/Office-2010/Setup.charm'), 'exe_name': #'Setup.exe', 'pid': 158991}}
        #Searching for script with label: Wine
        #Found matching row for script: Wine
        #Searching for script with label: Popcorn-Time-0.5.1-win64-0.44.5-Setup
        #Found matching row for script: Popcorn-Time-0.5.1-win64-0.44.5-Setup
        #Searching for script with label: Setup
        #Found matching row for script: Setup

    def find_row_by_script_stem(self, script_stem):
        script_label = script_stem.replace('_', ' ')
        print(f"Searching for script with label: {script_label}")
        
        row = self.script_buttons.get(script_label)
        if row:
            print(f"Found matching row for script: {script_stem}")
        else:
            print(f"No matching row found for script: {script_stem}")
...
################################ above working highlight/unhilight but not reliable "setup.exe"
            ...
            if process_info:
                ...
                if exe_name and pid:
                    try:
                        ...
                        print(f"ps output for PID {pid}:", ps_output)  # Log the ps output
                        if ps_output and exe_name in ps_output:
                            row.add_css_class("highlighted")
                        else:
                            row.remove_css_class("highlighted")
                            self.running_processes.pop(script_name)
...
############################### New Attempt
    ...
    def find_row_by_exe_name(self, exe_name):
        print(f"Searching for script with exe_name: {exe_name}")
        
        row = self.script_buttons.get(exe_name)
        if row:
            print(f"Found matching row for exe_name: {exe_name}")
        else:
            print(f"No matching row found for exe_name: {exe_name}")
        
        return row


### pgrep is showing no process but setup.exe popcorn-setup.exe issue
    def check_running_processes_and_update_buttons(self):
        if not self.monitoring_active:
            self.stop_monitoring()
            return False  # Stop the loop if not active
        ...
        try:
            ...
            print("pgrep: No running processes found")
            # If no running processes found, unhighlight all and clear running_processes
            for script_key, process_info in self.running_processes.items():
...
############################## spawn children are not highlighted and launch_button icon is not updated
    ...
    def check_running_processes_and_update_buttons(self):
        ...
        try:
            # Capture all running processes with their command-line arguments
            running_processes_output = subprocess.check_output(["ps", "-eo", "pid,comm,args"]).decode().splitlines()
            current_running_processes = {}  # Dictionary to track currently running processes
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    # Match processes where the command ends with the exact exe_name
                    relevant_processes = [
                        process for process in running_processes_output
                        if process.strip().endswith(f"wine {exe_name}")
                    ]
                    ...
                    if relevant_processes:
                        for process in relevant_processes:
                            ...
                            if row is not None:
                                ...
                                print(f"No row found for exe_name: {exe_name}")  # Debugging line
                    else:
                        # If the process has ended, unhighlight the row
                        if script_key in self.running_processes:
                            print(f"Process ended for script: {script_key} : {exe_name}")
            ...
            # Handle the case where no processes are found in the running process list
            if not current_running_processes and self.running_processes:
                print("No processes matched. Clearing highlights.")
                for script_key, process_info in self.running_processes.items():
                    row = process_info["row"]
                    if row is not None:
                        row.remove_css_class("highlighted")
...
############################################## works faster, but setup issue
    ...
    def check_running_processes_and_update_buttons(self):
        ...
        try:
            # Step 1: Capture all relevant processes using pgrep
            pgrep_output = subprocess.check_output(["pgrep", "-af", r"\.exe"]).decode().splitlines()
            current_running_processes = {}

            # Step 2: Loop over all scripts to check against captured processes
            for script in self.find_python_scripts():
                try:
                    exe_file, _, _, _ = self.extract_yaml_info(script)
                    exe_name = Path(exe_file).name
                    ...
                    # Check if the exe_name is in any of the pgrep output lines
                    matching_processes = [line for line in pgrep_output if exe_name in line]

                    if matching_processes:
                        pid = int(matching_processes[0].split()[0])  # Get PID from the first matching process
                        row = self.find_row_by_exe_name(exe_name)
                        ...
                        if row is not None:
                            ...
                            print(f"No row found for exe_name: {exe_name}")
                    else:
                        # If the process has ended, unhighlight the row
                        if script_key in self.running_processes:
                            print(f"Process ended for script: {script_key} : {exe_name}")
            ...
            # Step 3: Cleanup old processes no longer running
            for script_key, process_info in list(self.running_processes.items()):
                if script_key not in current_running_processes:
                    row = process_info["row"]
                    if row is not None:
                        row.remove_css_class("highlighted")
                    print(f"Cleaning up ended process for script: {script_key}")
                    self.running_processes.pop(script_key)
...
######################################################### much like before, setup issue rest fast
    def check_running_processes_and_update_buttons(self):
        if not self.monitoring_active:
            self.stop_monitoring()
            return False  # Stop the loop if not active
        ...
        try:
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    # Match the process in pgrep output that contains the full command with exe_name
                    matching_processes = [
                        line for line in pgrep_output
                        if exe_name_pattern.search(line)
                    ]

                    if matching_processes:
                        # Ensure the match is not just a part of another process name
                        for process in matching_processes:
                            # Extract command-line arguments to ensure the process belongs to our script
                            if exe_name in process:
                                pid = int(process.split()[0])  # Get PID from the first matching process
                                row = self.find_row_by_exe_name(exe_name)
                                ...
                                print(f"Process name mismatch: expected {exe_name}, got {process}")
                    else:
                        # If the process has ended, unhighlight the row
                        if script_key in self.running_processes:
                            print(f"Process ended for script: {script_key} : {exe_name}")
...
############################################## selects properly but same setup
    def check_running_processes_and_update_buttons(self):
        if not self.monitoring_active:
            self.stop_monitoring()
            return False  # Stop the loop if not active
        ...
        try:
            # Capture all relevant processes using pgrep
            pgrep_output = subprocess.check_output(["pgrep", "-af", r"\.exe"]).decode().splitlines()
            ps_output = subprocess.check_output(["ps", "-ax", "-o", "pid,command"]).decode().splitlines()
            current_running_processes = {}
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    matching_processes = [
                        line for line in pgrep_output
                        if re.search(rf"\b{re.escape(exe_name)}\b", line)
                    ]

                    # If pgrep didn't find the process, fall back to ps
                    ...
                    if matching_processes:
                        for process in matching_processes:
                            ...
                            # Ensure the full command contains the exact exe_name, not a substring match
                            if re.search(rf"\b{re.escape(exe_name)}\b", command_line):
                                row = self.find_row_by_exe_name(exe_name)
                                ...
                                print(f"Process name mismatch: expected {exe_name}, got {command_line}")
                    else:
                        # If the process has ended, unhighlight the row
                        if script_key in self.running_processes:
                            print(f"Process ended for script: {script_key} : {exe_name}")
            ...
            # Cleanup old processes no longer running
            for script_key, process_info in list(self.running_processes.items()):
                if script_key not in current_running_processes:
                    row = process_info["row"]
                    if row is not None:
...
################################################ FINALLY :-D
        ...
        try:
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    # Regex pattern to match the executable name with specific preceding characters
                    exe_name_pattern = re.compile(
                        rf"[\/\\\'\" ]{re.escape(exe_name)}\b"
                    )
                    ...
                    if matching_processes:
                        for process in matching_processes:
                            ...
                            # Ensure the full command contains the exact exe_name, not a substring match
                            if exe_name_pattern.search(command_line):
                                row = self.find_row_by_exe_name(exe_name)
                        ...
                        # If the process has ended, unhighlight the row
                        # RIZVAN end process stuff here like create script with lnk
                        if script_key in self.running_processes:
                            print(f"Process ended for script: {script_key} : {exe_name}")
                            print("========= if script_key in self.running_processes: ==========")
                            row = self.find_row_by_exe_name(exe_name)
                            if row is not None:
                                row.remove_css_class("highlighted")
                            self.running_processes.pop(script_key)
            ...
            for script_key, process_info in self.running_processes.items():
                row = process_info["row"]
                print("========= for script_key, process_info in self.running_processes.items(): ==========")
                print(row)
                if row is not None:
                    row.remove_css_class("highlighted")
            self.running_processes.clear()
    ...
    def terminate_script(self, script):
        try:
            ...
            # Use pgrep to find all PIDs related to the executable name
            pgrep_output = subprocess.check_output(["pgrep", "-af", exe_name]).decode().splitlines()

            # Regex pattern to match the executable name with specific preceding characters
            exe_name_pattern = re.compile(rf"[\/\\\'\" ]{re.escape(exe_name)}\b")
            ...
            if matching_pids:
                print(f"Terminating script: {script} with PIDs: {matching_pids}")
                for pid in matching_pids:
                    os.kill(pid, signal.SIGTERM)
                print(f"Successfully terminated script: {script}")
            else:
                print(f"No matching PIDs found for {exe_name}")

        except Exception as e:
            print(f"Error terminating script {script}: {e}")

############################ Now try creaet_scripts_for_lnk
