...
class WineCharmApp(Gtk.Application):
    ...
    def on_startup(self, app):
        ...
        self.update_ui_based_on_process_state()
        ...
        self.start_polling_window_state()

    def start_polling_window_state(self):
        """Periodically check if the window is active."""
        def check_window_active():
            if self.window and self.window.is_active():
                self.on_focus_in(None)
            return True  # Continue polling

        GLib.timeout_add_seconds(1, check_window_active)

    def on_window_active_changed(self, window, param_spec):
        if window.is_active():
            self.on_focus_in(None)
        else:
    ...
    def on_focus_in(self, controller):
        """
        Start monitoring when the window gains focus.
        """
        if self.monitoring_active:
            return  # Prevent multiple activations if already active

        print("Focus In: Starting monitoring")
        self.monitoring_active = True
        self.monitoring_count = 0  # Reset the counter each time focus is gained
        self.start_monitoring()


    def on_focus_out(self, controller):
        """
        Stop monitoring when the window loses focus.
        """
        if not self.monitoring_active:
            return  # Already inactive

        print("Focus Out: Stopping monitoring")
        self.stop_monitoring()


    def start_monitoring(self, delay=2):
        """
        Start the monitoring process, run every 'delay' seconds, up to 10 times.
        """
        def monitor_callback():
            ...
            print(f"Monitoring iteration {self.monitoring_count + 1}")
            self.update_ui_based_on_process_state()
        ...
        if self.monitoring_active:
            self._monitoring_id = GLib.timeout_add_seconds(delay, monitor_callback)


    def stop_monitoring(self):
        """
        Stop the monitoring process if it's currently active.
        """
    ...
    def on_script_row_clicked(self, button, play_button, options_button):
        ...
        using the `is_clicked_row` property within `script_ui_data`.
        ...
        script_key = None
        for key, info in self.script_ui_data.items():
        ...
        for key, data in self.script_ui_data.items():
            ...
            is_running = key in self.running_processes
...
#
#                # Only remove 'highlighted' if not running
                if not is_running:
...
######################## 4
    ...
    def process_ended(self, script_key):
        """
        Handles the end of a process, ensuring all related UI elements are reset.
        """
        ...
        # Retrieve row, play_button, and options_button from the UI state
        ...
        # Hide the play and options buttons and reset the play button to 'Play'
        ...
        if script_key in self.running_processes:
            ...
            # Create Shortcuts for found lnk files
            if wineprefix:
    ...
    def reset_all_ui_elements(self):
        ...
        """
        # Iterate over each script_key in the script_ui_data to reset elements
        for script_key, ui_state in self.script_ui_data.items():
            # Retrieve UI components related to the script
            row = ui_state.get('row')
            play_button = ui_state.get('play_button')
            options_button = ui_state.get('options_button')

            # Reset row highlight (remove any highlight classes)
            if row:
                # Ensure the row is un-highlighted and no special styles remain
                row.remove_css_class("highlighted")
                row.remove_css_class("blue")
                self.update_row_highlight(row, False)  # Reset highlight state
            ...
            # Set play button back to its default state (usually "Play")
            if play_button:
                self.set_play_stop_button_state(play_button, False)
        ...
        # Clear information about the current clicked row
        self.current_clicked_row = None

        # Clear all entries from the running_processes dictionary
        self.running_processes.clear()
    ...
    def terminate_script(self, script_key):
        ...
        try:
            if wineprefix_process_count == 1 and existing_running_script:
                ...
                # Run the command in a full bash shell environment
    ...
    def get_running_processes(self):
        ...
        # Create a copy of the script_list to avoid modifying it while iterating
    ...
    def cleanup_ended_processes(self, current_running_processes):
        ...
        self.update_ui_for_running_process(self, current_running_processes)
...
#        """)
    ...
    def on_remove_desktop_shortcut_response(self, dialog, response_id, checkbox_dict):
        ...
        dialog.close()
################
    def update_ui_based_on_process_state(self):
        """
        Update all UI elements based on the current state of running processes.
        Highlights rows for running processes, resets buttons for stopped processes, and ensures the UI is updated accordingly.
        """
        ...
        for script_key, ui_state in self.script_ui_data.items():
            ...
            is_running = script_key in current_running_processes
            is_clicked = ui_state.get('is_clicked_row', False)

            # Update UI for running processes
            if is_running:
                # If the script is running, highlight the row and set play button to "Stop"
                if not ui_state['is_running']:  # Only update if the current state is not already running
                    if row:
                        self.update_row_highlight(row, True)
                        row.add_css_class("highlighted")
                    ...
                    # Update internal state to indicate that this script is running
                    ui_state['is_running'] = True

            # Update UI for non-running processes
            else:
                if ui_state['is_running']:  # Only update if the current state is marked as running
                    # If the script is not running, reset UI elements to default state only if not clicked
                    if row:
                        # Remove 'highlighted' if the row is not clicked
                        if not is_clicked:
                            self.update_row_highlight(row, False)
                            row.remove_css_class("highlighted")
                            print(f"Removed 'highlighted' from row for script_key: {script_key}")

                    # Only reset the play button if the row is not clicked
                    if play_button and not is_clicked:
                        self.set_play_stop_button_state(play_button, False)

                    # Update internal state to indicate that this script is no longer running
                    ui_state['is_running'] = False

            # Handle the case where the row is clicked (regardless of running state)
            if is_clicked:
                if row:
                    row.add_css_class("blue")  # Ensure clicked row is highlighted in blue
                    print(f"Preserving 'blue' highlight for clicked row for script_key: {script_key}")

                if play_button and options_button:
                    # Show the buttons and keep the play button in the appropriate state based on running status
                    self.show_buttons(play_button, options_button)
                    self.set_play_stop_button_state(play_button, is_running)
                    print(f"Updated play/stop button for clicked row with script_key: {script_key}")

        # Update the launch button state if it matches the script_key
        if self.launch_button and getattr(self, 'launch_button_exe_name', None):
            launch_script_key = getattr(self, 'launch_button_exe_name')
            if launch_script_key in current_running_processes:
                self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-stop-symbolic"))
                self.launch_button.set_tooltip_text("Stop")
            else:
                self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-start-symbolic"))
                self.launch_button.set_tooltip_text("Play")
            print(f"Updated launch button for script_key: {launch_script_key}")

        # If no current running processes exist, reset all UI elements
        #if not current_running_processes:
            #self.reset_all_ui_elements()
    ...
    def reset_all_ui_elements(self):
        ...
        for script_key, ui_state in self.script_ui_data.items():
            ...
            if row:
                ...
                # Only remove blue if the row is not clicked
...
#                else:
            ...
            ui_state['is_running'] = False
            # Preserve `is_clicked_row` state if it is true
            if not ui_state.get('is_clicked_row', False):
                ui_state['is_clicked_row'] = False
