...
class WineCharmApp(Gtk.Application):
    def __init__(self):
        ...
        self.script_info = {}  # Dictionary to hold script information including sha256sum
        ...
        if process_info:
            ...
            if script_path and script_path.exists():
                ...
                if wineprefix:
                    ...
                    # Run the script creation in a background thread to avoid blocking the main process
                    #threading.Thread(target=self.create_scripts_for_lnk_files, args=(wineprefix_path,)).start()
                    print("self.create_scripts_for_lnk_files(wineprefix_path) process_ended")
                    self.create_scripts_for_lnk_files(wineprefix_path)
            # Ensure no further process check is triggered
            if script_key in self.running_processes:
                del self.running_processes[script_key]
...
######## Refine
    def create_script_list(self):
        ...
        scripts = self.find_python_scripts()
        for script in scripts:
            # Extract yaml info and store it in the script_info dictionary
            exe_file, wineprefix, progname, script_args, sha256sum = self.extract_yaml_info(script)
            self.script_info[script.stem] = {
                'exe_file': exe_file,
    ...
    def create_script_row(self, script):
        # Extract the sha256sum from the script_info dictionary
        sha256sum = self.script_info[script.stem]['sha256sum']
        ...
        # Store the button in the dictionary using sha256sum as the key
        self.script_buttons[sha256sum] = button

        return button

    def update_script_button_state(self, script_stem):
        # Use sha256sum as script_key
        script_key = self.script_info[script_stem]['sha256sum']
    ...
    def find_row_by_script_stem(self, script_stem):
        # Use sha256sum to retrieve the correct row
        script_key = self.script_info[script_stem]['sha256sum']
    ...
    def extract_yaml_info(self, script):
        ...
        # If sha256sum is missing, generate it and update the YAML file
        if not sha256sum:
            sha256sum = self.generate_sha256sum(script)
            data['sha256sum'] = sha256sum
            with open(script, 'w') as file:
                yaml.dump(data, file)

        return exe_file, wineprefix, progname, script_args, sha256sum

    def generate_sha256sum(self, script):
        sha256_hash = hashlib.sha256()
    ...
    def create_yaml_file(self, exe_path, prefix_dir=None, use_exe_name=False):
        try:
            ...
            print(f"Creating YAML for {exe_file}, resolved name: {exe_name}")
            ...
            if prefix_dir is None:
                prefix_dir = prefixes_dir / f"{exe_no_space}-{sha256sum}"
                print(f"Prefix directory set to: {prefix_dir}")
                if not prefix_dir.exists():
                    if default_template.exists():
                        self.copy_template(prefix_dir)
                        print(f"Template copied to {prefix_dir}")
                    else:
                        prefix_dir.mkdir(parents=True, exist_ok=True)
                        print(f"Created prefix directory: {prefix_dir}")
            ...
            # Debugging information for product command execution
            product_cmd = [
                'exiftool', shlex.quote(str(exe_file))
            ]
            print(f"Running product command: {' '.join(product_cmd)}")
            ...
            if product_output is None:
                ...
                productname_match = re.search(r'Product Name\s+:\s+(.+)', product_output)
                productname = productname_match.group(1).strip() if productname_match else exe_no_space
                print(f"Product name extracted: {productname}")
            ...
            print(f"Final program name: {progname}")
            ...
            self.create_desktop_entry(progname, yaml_file_path, icon_path, prefix_dir)
            print(f"Desktop entry created for {progname}")

            self.add_or_update_script_row(yaml_file_path)
            print(f"Script row added/updated for {yaml_file_path}")
            self.create_script_list()

        except Exception as e:
            print(f"Error processing file in create_yaml_file: {e}")
    ...
    def process_file(self, file_path):
        try:
            ...
            # Add debugging output to trace the process
            print(f"Starting to process the file: {abs_file_path}")

            self.create_yaml_file(abs_file_path, None)

            # Debug after YAML creation
            print(f"YAML file created successfully for {abs_file_path}")

            self.create_script_list()

            # Debug after script list creation
            print(f"Script list created/updated successfully for {abs_file_path}")

            print(f"Successfully processed file: {abs_file_path}")

        except Exception as e:
            # Catch any exceptions and print a detailed error message
            print(f"Error processing file in process_file: {e}")
        finally:
            GLib.idle_add(self.hide_processing_spinner)
    ...
    def copy_template(self, prefix_dir):
        try:
            print(f"Copying default template to {prefix_dir}")
            shutil.copytree(default_template, prefix_dir, symlinks=True)
            print(f"Template successfully copied to {prefix_dir}")
        except shutil.Error as e:
            print(f"Error copying template: {e}")
            for src, dst, err in e.args[0]:
                if not os.path.exists(dst):
            ...
            print(f"General error copying template: {e}")
    ...
    def create_yaml_file(self, exe_path, prefix_dir=None, use_exe_name=False):
        try:
            ...
            self.create_script_list()
            print(f"Script list created/updated successfully.")
