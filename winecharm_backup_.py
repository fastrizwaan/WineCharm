...
#           icon_symlink_path.symlink_to(icon_path)
    ...
    def toggle_play_stop(self, script, play_stop_button, button):
        self.launch_click_count += 1  # Increment the click count
        print(f"Launch button clicked {self.launch_click_count} times.")

        # Ensure that the button is not clicked multiple times by disabling it briefly
        play_stop_button.set_sensitive(False)
        GLib.timeout_add(500, lambda: play_stop_button.set_sensitive(True))  # Re-enable after 500ms

        if script.stem in self.running_processes:
            print(f"Stopping script: {script.stem} (Click {self.launch_click_count})")
            self.terminate_script(script)
            stop_icon = Gtk.Image.new_from_icon_name("media-playback-start-symbolic")
            play_stop_button.set_child(stop_icon)
            play_stop_button.set_tooltip_text("Play")
            print(f"Button functionality set to 'Launch' after stopping script. (Click {self.launch_click_count})")
        else:
            print(f"Starting script: {script.stem} (Click {self.launch_click_count})")
            self.launch_script(script, play_stop_button, button)
            start_icon = Gtk.Image.new_from_icon_name("media-playback-stop-symbolic")
            play_stop_button.set_child(start_icon)
            play_stop_button.set_tooltip_text("Stop")
            print(f"Button functionality set to 'Stop' after starting script. (Click {self.launch_click_count})")
    ...
    def reset_buttons(self, play_button, play_icon, script):
        print(f"Resetting buttons for script: {script.stem}")
        play_button.set_child(play_icon)
        play_button.set_sensitive(True)
        if play_button in self.play_stop_handlers:
            old_handler_id = self.play_stop_handlers.pop(play_button)
            print(f"Disconnecting old handler: {old_handler_id}")
            play_button.disconnect(old_handler_id)
        
        handler_id = play_button.connect("clicked", lambda btn: self.toggle_play_stop(script, play_button, self.find_button_by_script_stem(script.stem)))
        self.play_stop_handlers[play_button] = handler_id
        print(f"Button functionality reset to 'Launch' for script: {script.stem}")
        print(f"New handler connected: {handler_id}")
        print(f"Current handler: {self.play_stop_handlers[play_button]}")

    def launch_script(self, script, play_button, button):
        try:
            env = os.environ.copy()
            env['WINEPREFIX'] = "/default/wineprefix/path"  # Replace with your default wineprefix path

            process = subprocess.Popen(["wine", str(script)], env=env)
            self.running_processes[script.stem] = {
                "process": process,
                "play_button": play_button,
                "play_icon": Gtk.Image.new_from_icon_name("media-playback-start-symbolic"),
            ...
            print(f"Launching script {script} with PID {process.pid}")

        except Exception as e:
            print(f"Failed to launch script {script}: {e}")

    def update_related_scripts_buttons(self, wineprefix):
        print(f"Updating related scripts buttons for wineprefix: {wineprefix}")
        for script, process_info in self.running_processes.items():
            if process_info["wineprefix"] == wineprefix:
                self.reset_buttons(
                    process_info["play_button"],
                    process_info["play_icon"],
                    Path(script)
