...
# Constants
SOCKET_FILE = Path(os.path.expanduser("~/.var/app/io.github.fastrizwaan.WineCharm/data/winecharm/winecharm_socket"))
do_not_kill = "bin/winecharm"
prefixes_dir = Path(os.path.expanduser("~/.var/app/io.github.fastrizwaan.WineCharm/data/winecharm/Prefixes")).resolve()
version = "0.77+1"
...
class WineCharmApp(Gtk.Application):
    ...
    def on_kill_all_clicked(self, action=None, param=None):
        print("Kill all action triggered")
        # Add the logic to kill all running processes here.
    ...
    def monitor_process(self, script_key, exe_file):
        exe_name = Path(exe_file).name
