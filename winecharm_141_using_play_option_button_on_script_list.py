...
# import wine directory
    ...
    def clear_highlighted_rows(self):
        # Clear the highlighted state from all script rows
        for btn in self.script_buttons.values():
            btn.remove_css_class("highlighted")
    ...
    def create_script_row(self, script):
        ...
        # Use self.launch_button
        buttons_box.append(self.launch_button)
        ...
        # Event handler for button click
        button.connect("clicked", lambda *args: self.on_script_row_clicked(button, self.launch_button, options_button))
...
#        # Delete button for setup or install scripts
#        if "setup" in script.stem.lower() or "install" in script.stem.lower():
...
#            button_box.append(delete_button)
    ...
    def create_script_row(self, script):
        ...
        # Main button for the script row
        button = Gtk.Button()
        button.set_hexpand(True)
        button.set_halign(Gtk.Align.FILL)
        button.add_css_class("flat")
        ...
        # Create an overlay to add the button_box
        overlay = Gtk.Overlay()
        overlay.set_child(button)

        # Use the create_button_box method to generate the overlay buttons
        button_box = self.create_button_box(script, button)
        button_box.set_visible(False)  # Initially hidden

        # Add the button box as an overlay
        ...
        # Event handler for button click
        button.connect("clicked", lambda *args: self.on_script_row_clicked(button, button_box, button_box))

        # Hover effect to show/hide the overlay buttons
        hover_controller = Gtk.EventControllerMotion()
        hover_controller.connect("enter", lambda controller, x, y, button_box=button_box: button_box.set_visible(True))
        hover_controller.connect("leave", lambda controller, button_box=button_box: button_box.set_visible(False))
    ...
    def process_ended(self, script_key):
        ...
        if process_info:
            ...
            # Ensure launch_button_exe_name is checked safely
            if getattr(self, 'launch_button_exe_name', None) == script_key:
                self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-start-symbolic"))
                self.launch_button.set_tooltip_text("Play")
    ...
    def cleanup_ended_processes(self, current_running_processes):
        ...
        if not current_running_processes:
            for script_key, button in self.script_buttons.items():
                # Assuming each button has a play button associated with it
                row = self.script_buttons.get(script_key)
                if row:
                    play_button = row.get_first_child()
                    if play_button:
    ...
    def on_script_row_clicked(self, button, play_button, options_button):
        ...
        if self.current_clicked_row and self.current_clicked_row[0] == button:
            ...
            # Check if the script associated with this row is running
            script_key = self.get_script_key_from_button(button)  # Method to get the script key from the button
            if script_key in self.running_processes:
                self.set_play_stop_button_state(play_button, True)  # Set the play button to "Stop"
            else:
                self.set_play_stop_button_state(play_button, False)  # Reset the play button to "Play"
    ...
    def on_script_row_clicked(self, button, play_button, options_button):
            ...
            # Check if the button corresponds to a running process
            script_key = None
            for key, row in self.script_buttons.items():
                if row == button.get_parent():
                    script_key = key
                    break
            ...
            if script_key and script_key in self.running_processes:
                ...
                self.set_play_stop_button_state(play_button, False)  # Otherwise, reset to "Play"
