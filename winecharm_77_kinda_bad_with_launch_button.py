...
class WineCharmApp(Gtk.Application):
    ...
    def start_monitoring(self):
        ...
        if not hasattr(self, '_monitoring_id'):
            self._monitoring_id = GLib.timeout_add_seconds(5, self.check_running_processes_and_update_buttons)
...
##NEW
    ...
    def check_running_processes_and_update_buttons(self):
        ...
        try:
            # Capture all relevant processes using pgrep and ps
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    matching_processes = [line for line in pgrep_output if exe_name_pattern.search(line)]
                    if matching_processes:
                        ...
                        # Handle the case where the process has ended
                        self.process_ended(script_key, exe_name, wineprefix)
            ...
            # Handle the case where no processes are found
            self.handle_no_processes_found()
    ...
    def cleanup_ended_processes(self, current_running_processes):
        for script_key, process_info in list(self.running_processes.items()):
            if script_key not in current_running_processes:
                ...
                print("cleanup_ended_processes................................")
                if script_path.exists():
                    print("WINEPREFIX INSIDE cleanup_ended_processes................................")
                    wineprefix = self.extract_yaml_info(script_path)[1]
                    if wineprefix:
                        self.create_scripts_for_lnk_files(Path(wineprefix))
    ...
    def update_script_row(self, script):
        row = self.find_row_by_script_stem(script.stem)
        if row:
            self.flowbox.invalidate_filter()  # Or use another GTK method to refresh the specific row


    def get_wine_charm_pids(self):
        if not hasattr(self, '_winecharm_pids_cache'):
            try:
                winecharm_pid_output = subprocess.check_output(["pgrep", "-af", do_not_kill]).decode()
                self._winecharm_pids_cache = {int(line.split()[0]) for line in winecharm_pid_output.splitlines()}
            except subprocess.CalledProcessError:
                self._winecharm_pids_cache = set()
        return self._winecharm_pids_cache
    ...
    def load_icon(self, script):
        ...
        try:
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(str(icon_path), 32, 32)
            icon = Gdk.Texture.new_for_pixbuf(pixbuf)
    ...
    def process_ended(self, script_stem, *args):
        """Handles actions after a script's process has ended."""
        try:
            process_info = self.running_processes.get(script_stem)

            if process_info:
                if process_info.get("ended_handled", False):
                    return  # Skip if already handled

                process_info["ended_handled"] = True
                print(f"Process for script {script_stem} has ended.")
                ...
                del self.running_processes[script_stem]
            else:
                pass #print(f"No running process found for script: {script_stem}")

        except Exception as e:
            print(f"Error in process_ended for {script_stem}: {e}")
    ...
    def replace_open_button_with_launch(self, script, row):
        ...
        # Safely check if the `launch_button` exists and is added to the UI
        if hasattr(self, 'launch_button') and self.launch_button and self.launch_button.get_parent():
            self.vbox.remove(self.launch_button)

        # Create a new "Launch" button with the icon centered
        self.launch_button = Gtk.Button()
        self.launch_button.set_size_request(390, 36)  # Adjust the size as needed

        # Calculate the script key using the script path
        script_key = hashlib.sha256(str(script).encode()).hexdigest()

        # Set the "Launch" icon based on whether the script is running
        if script_key in self.running_processes:
        ...
        # Connect the "Launch" button to start/stop the script
        ...
        if script_key in self.running_processes:
            ...
            try:
                if pid:
                    ...
                    print(f"Terminated script: {script}, PID: {pid}")
                
                # Clean up the running process entry
...
#                    return  # Skip if already handled
                ...
                if row:
                    ...
                    # Update the launch button to reflect the stopped state
                ...
                print(f"No running process found for script: {script_stem}")
