        ...
        try:
            ...
            for script in self.find_python_scripts():
                try:
                    ...
                    if matching_processes:
                        ...
                        # Process ended, handle script post-processing
                        if script_key in self.running_processes:
                            ...
                            if row is not None:
                                row.remove_css_class("highlighted")
                                # Now call `create_scripts_for_lnk_files` for this wineprefix
                                print(f"wineprefix={wineprefix}")
                                if wineprefix:
                                    print("lnk files 1111111111111111111111111111")
            ...
            for script_key, process_info in list(self.running_processes.items()):
                if script_key not in current_running_processes:
                    ...
                    wineprefix = None  # Initialize wineprefix to avoid UnboundLocalError
                    if row is not None:
                        row.remove_css_class("highlighted")
                    script_path = process_info["script"]
                    if script_path.exists():  # Check if the script file exists before extracting info
                        wineprefix = self.extract_yaml_info(script_path)[1]
                        if wineprefix:
                            print("lnk files 22222222222222222222222222222")
                            self.create_scripts_for_lnk_files(Path(wineprefix))
                    else:
                        print(f"Script file not found: {script_path}, skipping cleanup.")
            ...
            for script_key, process_info in self.running_processes.items():
                ...
                if row is not None:
                    row.remove_css_class("highlighted")
                    script_path = process_info["script"]
                    if script_path.exists():  # Check if the script file exists before 
                        wineprefix = self.extract_yaml_info(script_path)[1]
                        if wineprefix:
                            print("lnk files 3333333333333333333333333333333")
                            self.create_scripts_for_lnk_files(Path(wineprefix))
                else:
                    print(f"Script file not found: {script_path}, skipping cleanup.")
            self.running_processes.clear()
