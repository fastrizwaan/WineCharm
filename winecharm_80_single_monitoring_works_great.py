...
class WineCharmApp(Gtk.Application):
    ...
    def start_monitoring(self):
        if not hasattr(self, '_monitoring_id'):
            self._monitoring_id = GLib.timeout_add_seconds(3, self.check_running_processes_and_update_buttons)
    ...
    def launch_script(self, script, play_stop_button, row):
        ...
        try:
            ...
            #threading.Thread(target=self.monitor_process, args=(script_key, exe_file)).start()
                    ...
                    for process in matching_processes:
                        ...
                        if exe_name_pattern.search(command_line):
                            ...
                            if self.launch_button:
                                print("Hello from process ended: self.launch_button")
                                self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-stop-symbolic"))
                                self.launch_button.set_tooltip_text("Play Set by launch_button")
                else:
                    print("self.process_ended(script_key), can we use create_script_for_lnk_files?")
        ...
        for script_key, process_info in list(self.running_processes.items()):
            if script_key not in current_running_processes:
                ...
                if self.launch_button:
                    print("Hello from process ended: self.launch_button")
                    self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-stop-symbolic"))
                    self.launch_button.set_tooltip_text("Stop Shaz")
                    del self.running_processes[script_key]
    ...
    def create_yaml_file(self, exe_path, prefix_dir=None):
        ...
        yaml_data = {
            ...
            'progname': exe_no_space,
            'args': "",
            'sha256sum': sha256_hash.hexdigest()
        }
        
        yaml_file_path = prefix_dir / f"{exe_no_space}.charm"
...
################
    def process_ended(self, script_key):
        ...
        if process_info:
            ...
            if row:
                row.remove_css_class("highlighted")
                print("Hello from process ended: remove_css_class")
            if self.launch_button:
                print("Hello from process ended: self.launch_button")
                self.launch_button.set_child(Gtk.Image.new_from_icon_name("media-playback-start-symbolic"))
                self.launch_button.set_tooltip_text("Play")
...
#####################
    def update_running_script_buttons(self):
        for script_key, process_info in self.running_processes.items():
            row = process_info.get("row")
            if row:
                # Retrieve the play button for this row
                play_button = process_info.get("play_button")
                if play_button:
                    # Check if the play button currently has a "stop" icon
                    current_icon = play_button.get_child()
                    if current_icon.get_icon_name() == "media-playback-stop-symbolic":
                        # If it has a stop icon, toggle to play icon
                        play_icon = Gtk.Image.new_from_icon_name("media-playback-start-symbolic")
                        play_button.set_child(play_icon)
                        play_button.set_tooltip_text("Play")
                        row.remove_css_class("highlighted")
                    else:
                        # If it has a play icon, toggle to stop icon
                        stop_icon = Gtk.Image.new_from_icon_name("media-playback-stop-symbolic")
                        play_button.set_child(stop_icon)
                        play_button.set_tooltip_text("Stop")
                        row.add_css_class("highlighted")
                else:
                    print(f"No play button found for script: {script_key}")
            else:
                print(f"No row found for script: {script_key}")
