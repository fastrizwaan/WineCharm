...
################ Terminate script with wineserver -k but this will kill all running exe in the wineprefix
# need to filter out 
    ...
    def get_running_processes(self):
        ...
        try:
            ...
            for script in self.find_python_scripts():
                ...
                # If `winedbg` fails, fall back to using the `pgrep` results
                try:
                    if len(matching_processes) > 1:
                        wine_processes = self.get_wine_processes(wineprefix, exe_name)
                        print(f"Refined processes using winedbg for {script.stem}: {wine_processes}")  # Debugging output
                        if wine_processes:
                            matching_processes = wine_processes
                except Exception as e:
                    print(f"Error running winedbg: {e}")
                    # Keep using `pgrep` output if `winedbg` fails
...
#####################################
    ...
    def get_running_processes(self):
        ...
        try:
            # Fetch all processes with .exe in their command line
            pgrep_output = subprocess.check_output(["pgrep", "-aif", "\\.exe"]).decode().splitlines()
            print(f"pgrep output: {pgrep_output}")  # Debugging output
            ...
            for script in self.find_python_scripts():
                ...
                # Match processes by executable name and working directory to avoid duplicates
                matching_processes = [
                    (int(line.split()[0]), line.split(None, 1)[1]) for line in pgrep_output
                    if exe_name in line and self.is_process_in_correct_directory(line, exe_full_path)
                ]

                # Log the initial matching processes for debugging
                print(f"Initial matching processes for {script.stem}: {matching_processes}")

                if len(matching_processes) > 1:
                    try:
                        # Attempt to refine using winedbg
                        wine_processes = self.get_wine_processes(wineprefix, exe_name)
                        print(f"Refined processes using winedbg for {script.stem}: {wine_processes}")
                        if wine_processes:
                            matching_processes = wine_processes
                    except Exception as e:
                        print(f"Error running winedbg: {e}")
                        # Fallback to using initial `pgrep` results if `winedbg` fails
    ...
    def is_process_in_correct_directory(self, process_line, exe_full_path):
        """
        Checks if the process is running in the correct directory.
        This is used to avoid highlighting duplicate processes with the same executable name.
        """
        try:
            pid = int(process_line.split()[0])
            proc_cwd = os.readlink(f"/proc/{pid}/cwd")
            expected_dir = str(Path(exe_full_path).parent)
            return proc_cwd == expected_dir
        except Exception as e:
            print(f"Error checking process directory: {e}")
            return False
    ...
    def get_running_processes(self):
        ...
        try:
            ...
            for script in self.find_python_scripts():
                ...
                # Find processes that match the exe_name and the parent directory name
                matching_processes = [
                    (int(line.split()[0]), line.split(None, 1)[1]) for line in pgrep_output
