...
class WineCharmApp(Adw.Application):
    def __init__(self):
        ...
        self.hamburger_actions = [
            ...
            ("📥 Process WineZGUI Scripts...", self.process_winezgui_sh_files),
...
###################### 0.95
    ...
    def update_script_list(self, temp_script_list, clear_existing=True):
        self.print_method_name()
        if clear_existing:
            self.script_list = temp_script_list
        else:
            self.script_list = {**temp_script_list, **self.script_list}
        print(f"Loaded {len(self.script_list)} scripts.")
        self.create_script_list()
        ...
        def process_in_background():
            try:
                ...
                for subdir in self.winezgui_prefixes_dir.iterdir():
                    if subdir.is_dir():
                        sh_files.extend(subdir.glob("*.sh"))
                ...
                for sh_file in sh_files:
                    if self.stop_processing:
                        break
                    sh_file = Path(sh_file)
                    prefix_dir = sh_file.parent
                    charm_file = self.process_sh_files(prefix_dir)
                    if charm_file:
                        created_count += 1
    ...
    def create_charm_from_sh(self, sh_file):
        self.print_method_name()
        """
        Create a .charm file from a WineZGUI .sh file in its prefix directory.
        """
        sh_file = Path(sh_file)
        prefix_dir = sh_file.parent
        ...
        try:
            ...
            # Extract WINEPREFIX
            wineprefix_match = re.search(r'WINEPREFIX=[\'"]?([^\'"\n;]+)[\'"]?', content)
            wineprefix = Path(wineprefix_match.group(1)).expanduser().resolve() if wineprefix_match else prefix_dir
            wineprefix_str = self.replace_home_with_tilde_in_path(str(wineprefix))

            # Extract executable and arguments
            exe_match = re.search(r'wine\s+([^\s;]+(?:\s+[^;\n]*)*?)(?:;|$)', content, re.MULTILINE)
            exe_file = None
            args = ""
            if exe_match:
                command = exe_match.group(1).strip()
                parts = shlex.split(command)
                if parts:
                    exe_file = Path(parts[0]).expanduser().resolve()
                    if not exe_file.exists():
                        print(f"Executable {exe_file} does not exist, skipping {sh_file}")
                        return None
                    args = " ".join(parts[1:]) if len(parts) > 1 else ""

            if not exe_file:
                print(f"No valid executable found in {sh_file}")
                return None

            exe_file_str = self.replace_home_with_tilde_in_path(str(exe_file))

            # Extract runner
            runner = self.runner or "wine"
            path_match = re.search(r'export\s+PATH=[\'"]?([^:]+):', content)
            if path_match:
                runner_dir = Path(path_match.group(1)).expanduser().resolve()
                runner = runner_dir / "wine"
                if not runner.exists():
                    print(f"Runner {runner} does not exist, using default.")
                    runner = self.runner or "wine"
            runner_str = self.replace_home_with_tilde_in_path(str(runner)) if Path(runner).exists() else "wine"

            # Extract environment variables
            env_vars = []
            allowed_vars = ["DXVK_HUD", "MANGOHUD", "WINEDEBUG", "DXVK_LOG_LEVEL"]
            env_matches = re.findall(r'export\s+([A-Za-z_][A-Za-z0-9_]*)=([^\n;]+)', content)
            for var, value in env_matches:
                if var in allowed_vars:
                    env_vars.append(f"{var}={value.strip()}")
            env_vars_str = "; ".join(env_vars) if env_vars else ""

            # Program name
            progname = exe_file.stem
            progname = re.sub(r'^setup_', '', progname, flags=re.IGNORECASE)
            progname = progname.replace('_', ' ').title()
            ...
            charm_data = {
                ...
                "wineprefix": wineprefix_str,
                "template": self.replace_home_with_tilde_in_path(str(self.template or self.default_template_win64)),
                "arch": self.get_template_arch(self.template or self.default_template_win64),
                "mtime": int(datetime.now().timestamp())
            }

            # Generate SHA256 sum
            sha256sum = self.generate_sha256sum(charm_data)
            charm_data["sha256sum"] = sha256sum

            # Set script_path
            charm_file = prefix_dir / f"{sha256sum}.charm"
            charm_data["script_path"] = self.replace_home_with_tilde_in_path(str(charm_file))

            # Write charm file
            try:
                with self.file_lock:
                    with open(charm_file, 'w', encoding='utf-8') as f:
                        yaml.dump(charm_data, f, default_style="'", default_flow_style=False, width=10000)
                print(f"Created .charm file: {charm_file}")
                return charm_file
            except Exception as e:
                print(f"Error writing .charm file {charm_file}: {e}")
                return None
