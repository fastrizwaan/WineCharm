...
class WineCharmApp(Gtk.Application):
    def __init__(self):
        ...
        self.connect("open", self.on_open)  # Ensure the on_open method is connected
...
#        GLib.timeout_add_seconds(2, self.start_monitoring)
    ...
    def get_running_processes(self):
        """
        Retrieve and map currently running .exe processes to scripts.
        Returns a dictionary of running processes with relevant details.
        """
...
##################################
    def on_open(self, app, files, n_files, hint):
        for file in files:
            file_path = file.get_path()
            print(f"Processing file from on_open: {file_path}")
            self.process_cli_file(file_path)
...
##########################################
    ...
    def hide_processing_spinner(self):
        if self.spinner:
            if self.spinner.get_parent() == self.button_box:
                self.spinner.stop()
                self.button_box.remove(self.spinner)
    ...
    def show_processing_spinner(self, message="Processing..."):
        print("Starting spinner...")  # Debugging
        if not hasattr(self, 'spinner') or self.spinner is None:
            self.spinner = Gtk.Spinner()

        # Remove the spinner first if it is already in the button_box to avoid duplicates
        if self.spinner.get_parent() is not None:
            self.button_box.remove(self.spinner)
        ...
        # Force a UI refresh to ensure the label updates
        self.open_button.queue_draw()
        while GLib.MainContext.default().pending():
            GLib.MainContext.default().iteration()

    def hide_processing_spinner(self):
        print("Stopping spinner...")  # Debugging
        if hasattr(self, 'spinner') and self.spinner and self.spinner.get_parent() is self.button_box:
            self.spinner.stop()
            self.button_box.remove(self.spinner)
...
##########################
    def show_processing_spinner(self, message="Processing..."):
        ...
        # Instead of calling Gtk.main_iteration_do(), simply queue the spinner start to be processed
        GLib.idle_add(self.spinner.queue_draw)

    def hide_processing_spinner(self):
        print("Stopping spinner...")
        if hasattr(self, 'spinner') and self.spinner.get_parent() is self.button_box:
            self.spinner.stop()
            self.button_box.remove(self.spinner)
        ...
        # Queue the UI update to ensure it is processed in the main loop
        GLib.idle_add(self.open_button.queue_draw)
    ...
    def _process_cli_file(self, file_path):
        ...
        try:
            ...
            # Ensure the temp directory exists and is a directory
            print("Tempdir")
            print(tempdir)
            temp_dir = Path(self.tempdir)
            if not temp_dir.exists():
                ...
                print(f"Temp path is not a directory: {temp_dir}")
                return
            ...
            GLib.idle_add(self.check_and_handle_empty_flowbox)
            GLib.idle_add(self.hide_processing_spinner)


    def check_and_handle_empty_flowbox(self):
        # Check if the FlowBox has any children
        child = self.flowbox.get_first_child()
        if not child:
            print("No scripts found or processed.")
            self.show_no_scripts_message()

    def show_no_scripts_message(self):
        # Add a label or a button that indicates no scripts are available
        no_scripts_label = Gtk.Label(label="No scripts available. Please open a file to begin.")
        no_scripts_label.set_halign(Gtk.Align.CENTER)
        no_scripts_label.set_valign(Gtk.Align.CENTER)
        self.main_frame.set_child(no_scripts_label)
    ...
    def on_back_button_clicked(self, button):
        ...
        # Force a UI refresh to ensure all widgets are properly allocated
...
###########################
    def _process_cli_file(self, file_path):
        if not self.window:
            print("Application is not fully initialized. Deferring file processing...")
            GLib.idle_add(self._process_cli_file, file_path)
            return
    ...
    def on_open(self, app, files, n_files, hint):
        ...
        GLib.idle_add(self.on_startup, None)
        
        for file in files:
            file_path = file.get_path()
            print(f"Processing file from on_open: {file_path}")
            self.command_line_file = file_path
            self.process_cli_file(file_path)
...
#######################
    ...
    def show_no_scripts_message(self):
        ...
        no_scripts_label.set_valign(Gtk.Align.CENTER)
        self.main_frame.set_child(no_scripts_label)
        #self.window.present()  # Ensure the window is shown
        ...
        if SOCKET_FILE.exists():
            try:
                with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
                    ...
                    client.sendall(message.encode())
                    print(f"Sent file path to existing instance: {args.file}")
                return  # Exit after sending the file to the existing instance
            except ConnectionRefusedError:
                print("No existing instance found, starting a new one.")
        else:
            print("No existing instance found, starting a new one.")
