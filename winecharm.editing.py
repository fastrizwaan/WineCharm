...
################################## import wine directory progress
    def custom_copytree(self, src, dst, progress_range=(0.0, 1.0)):
        """
        Custom copy implementation with detailed progress tracking
        """
        try:
            ...
            total_files = len(out.decode().splitlines())
            if total_files == 0:
    ...
    def import_wine_directory(self, src, dst):
        """
        Import Wine directory with hierarchical progress tracking
        """
        self.stop_processing = False
        backup_dir = Path(dst).parent / f"{Path(dst).name}_backup_{int(time.time())}"
        
        # Step configuration with progress weights
        steps = [
            {
                "name": "Preparing backup",
                "weight": 0.1,
                "func": lambda: self.backup_existing_directory(dst, backup_dir)
            },
            {
                "name": "Copying directory", 
                "weight": 0.6,
                "func": lambda: self.custom_copytree(src, dst, (0.1, 0.7))
            },
            {
                "name": "Updating registry",
                "weight": 0.1,
                "func": lambda: self.process_reg_files(dst)
            },
            {
                "name": "Adjusting paths",
                "weight": 0.1,
                "func": lambda: self.perform_replacements(dst)
            },
            {
                "name": "Creating shortcuts", 
                "weight": 0.1,
                "func": lambda: self.create_scripts_for_exe_files(dst)
            }
        ]
        
        # Initialize UI
        GLib.idle_add(self.flowbox.remove_all)
        GLib.idle_add(self.show_processing_spinner, "Starting import...")
        GLib.idle_add(lambda: self.progress_bar.set_fraction(0.0))
        self.connect_open_button_with_import_wine_directory_cancel()
        ...
        def perform_import_steps():
            try:
                ...
                for step in steps:
                    ...
                    # Update step UI
                    GLib.idle_add(self.show_initializing_step, step["name"])
                    ...
                    GLib.idle_add(lambda: self.progress_bar.set_fraction(
                        min(current_progress / total_weight, 0.99)
                    ))
                    GLib.idle_add(self.mark_step_as_done, step["name"])
                ...
                GLib.idle_add(lambda: self.handle_import_error(dst, backup_dir, str(e)))
    ...
    def backup_existing_directory(self, path, backup_path):
        """Handle existing directory backup"""
        if Path(path).exists():
            self.custom_copytree(path, backup_path, (0.0, 0.1))
