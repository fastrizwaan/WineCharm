...
class WineCharmApp(Gtk.Application):
    def __init__(self):
        ...
        self.monitoring_active = False  # Flag to control monitoring
...
#self.window.present()
    ...
    def launch_script(self, script, play_stop_button, row):
        ...
        script_key = yaml_info['sha256sum']
        env_vars = yaml_info.get('env_vars', '')
        ...
        try:
            ...
            # Define a callback function to handle the result of get_wine_processes
            def on_wine_processes_found(processes):
                if processes:
                    pid = processes[0][0]  # Get the first matching process PID
                    ...
                    print(f"Wine process for {exe_name} started with PID: {pid}")

                    self.set_play_stop_button_state(play_stop_button, True)
                    self.update_row_highlight(row, True)
                else:
                    print(f"No Wine process found for {exe_name}")

            # Run get_wine_processes in a separate thread with the callback
            self.get_wine_processes(wineprefix, exe_name, callback=on_wine_processes_found)
    ...
    def get_wine_processes(self, wineprefix, exe_name, callback=None):
        def run_winedbg():
            try:
                ...
                if callback:
                    GLib.idle_add(callback, processes)  # Call the callback function on the main thread

            except subprocess.CalledProcessError as e:
                print(f"Error running winedbg: {e}")
                if callback:
                    GLib.idle_add(callback, [])  # Pass an empty list if there was an error

        # Start the thread
        thread = threading.Thread(target=run_winedbg)
        thread.start()
