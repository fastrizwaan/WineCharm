...
class WineCharmApp(Gtk.Application):
    def __init__(self):
        ...
        self.script_options_active = False  # Flag to track if script options view is active
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if keyval == Gdk.KEY_f and (state & Gdk.ModifierType.CONTROL_MASK) and not self.script_options_active:
            ...
            if self.search_active and not self.script_options_active:
                print("Escape detected, deactivating search mode")
                self.deactivate_search_mode()
            elif self.script_options_active:
                print("Escape detected, going back from script options")
                self.on_back_button_clicked(None)

    def on_search_button_clicked(self, button):
        if not self.script_options_active:
...
#           icon_symlink_path.symlink_to(icon_path)
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        ...
        if keyval == Gdk.KEY_f and (state & Gdk.ModifierType.CONTROL_MASK) and not self.script_options_active:
            print("Ctrl+F detected")
            if not self.search_active and not self.script_options_active:
                self.activate_search_mode()
            elif self.search_active and not self.script_options_active:
                self.deactivate_search_mode()
        elif keyval == Gdk.KEY_Escape:
            if self.search_active and not self.script_options_active:
                print("Escape detected, deactivating search mode")
    ...
    def on_search_button_clicked(self, button):
        if not self.script_options_active:
            ...
            if self.search_active and not self.script_options_active:
                self.deactivate_search_mode()
                self.script_options_active = False 
            elif not self.search_active and not self.script_options_active:
                self.activate_search_mode()
                self.script_options_active = False 
                print("False????????")
    ...
    def show_options_for_script(self, script, button):
        #self.search_button.set_active(False)
       # self.flowbox_state = False
        self.script_options_active = True  # Set script options active
        self.deactivate_search_mode()
        scrolled_window = Gtk.ScrolledWindow()
        scrolled_window.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
        scrolled_window.set_vexpand(True)
    ...
    def on_back_button_clicked(self, button):
        ...
        self.flowbox_state = False
        self.script_options_active = False  # Reset script options active
        self.activate_search_mode()
        self.deactivate_search_mode()
