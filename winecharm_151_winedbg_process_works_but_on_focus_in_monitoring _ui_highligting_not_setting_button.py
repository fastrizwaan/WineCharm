...
# import wine directory
    ...
    def get_child_pid(self, script_key, exe_name, wineprefix):
        try:
            # Run winedbg to get the process tree and filter for the exe_name
            grep_exe_name = shlex.quote(str(exe_name))
            winedbg_command = f"WINEPREFIX={wineprefix} winedbg --command 'info proc' | grep -A1 {grep_exe_name}"
            winedbg_output = subprocess.check_output(winedbg_command, shell=True, text=True).strip()
            #print("-----------------------------------------------")
            #print(f"output: {winedbg_output}")
            #print('===============================================')
#            winedbg_command = f"WINEPREFIX={wineprefix} winedbg --command \"info proc\" | grep -A1 \"{exe_name}\" | grep -v 'grep' | tail -n1 | sed 's/ *//g' | cut -f2- -d '_' | tr \"'\" ' '"
            
            # Using this
            winedbg_command = f"WINEPREFIX={wineprefix} winedbg --command \"info proc\" | grep -A1 {grep_exe_name} | grep -v 'grep' | tail -n1 | cut -f2- -d '_' | tr \"'\" ' '"

            winedbg_output = subprocess.check_output(winedbg_command, shell=True, text=True).strip()
            print("-----------------------------------------------")
            print(f"output: {winedbg_output}")

            # Use pgrep to get the child PID
#            pgrep_command = f"pgrep -af \"{winedbg_output}\" | cut -f1 -d ' '"
            print("-x-x-xx-x-x-x-x-x-x-x-xx-x-x-x-x-x-x-x-x-xx-x-x-x-")
            #pgrep_child_process_name = shlex.quote(winedbg_output[15:])
            #pgrep_child_process_name = winedbg_output[15:]
            #print(f"grep_child_process_name = {pgrep_child_process_name}")
            #pgrep_command = f"pgrep -af \"{pgrep_child_process_name}\" | cut -f1 -d ' '"
            
           # ps not working as expected
            #escaped_output = re.escape(winedbg_output)
            #pgrep_command = f"pgrep -af \"{escaped_output}\" | cut -f1 -d ' '"
            pgrep_command = f"ps -ax |grep \"{winedbg_output}\" | grep -v 'grep'| sed 's/^ *//g' | cut -f1 -d ' '"
            print(pgrep_command)
            pgrep_output = subprocess.check_output(pgrep_command, shell=True, text=True).strip()

            # Split multiple PIDs and get the first one
