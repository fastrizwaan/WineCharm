...
class WineCharmApp(Gtk.Application):
    ...
    def on_key_pressed(self, controller, keyval, keycode, state):
        if keyval == Gdk.KEY_f and (state & Gdk.ModifierType.CONTROL_MASK) and  self.flowbox_state:
            self.search_button.set_active(not self.search_active)
        elif keyval == Gdk.KEY_Escape and self.options_listbox:
...
#           icon_symlink_path.symlink_to(icon_path)
    ...
    def on_back_button_clicked(self, button):
        ...
        # Ensure the main_frame doesn't already have a child before setting a new one
        if self.main_frame.get_child() != self.scrolled:
            self.main_frame.set_child(None)  # Remove existing child if any
            self.main_frame.set_child(self.scrolled)
